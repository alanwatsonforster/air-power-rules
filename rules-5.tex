...
\addedin{1B}{FH in APJ 36 errata}{When an expenditure of $2/3$ of an aircraft's speed (or FPs) is required, use the $2/3$ entry from the chart, not $2 \times 1/3$. Drop all fractions read off the chart, unless the entry is 0.5, in which case read 1. In this case 1 FP qualifies as 2/3.}

\addedin{1B}{FH in APJ 36 errata}{\paragraph{Speed Requirements Split Between Game Turns.} Some game activities, like ground-attack aiming, are measured in fractions of an aircraft's speed. If such a period extends between two game-turns, the highest speed in either game-turn is used to compute the completion requirements in the second turn.}
...
Example Of Level Flight: An aircraft has a speed of 3.0. The player moves the aircraft through three hexes as shown \changedin{1D}{AWF}{below}{in Figure~\ref{figure:level-flight}.}\deletedin{1D}{AWF}{\addedin{1B}{JDW in the APJ 23 errata}{ [The diagram actually shows the left aircraft moving four hexes/hexsides. This is in error.]}} Its turn is over when it has finished moving.
...
\caption{\protect\x{Level Flight}{The two aircraft each expend three HFPs to move forward three hexes or hexsides.}}
\label{figure:level-flight}
\end{figure}
}
