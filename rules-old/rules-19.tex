...
\paragraph{External DDS.} External DDS pods are identified \changedin{1B}{1B-tables}{on the External Pod Table}{in Table~\ref{table:stores-EP}}, which shows the possible loads of decoys available.
...
\paragraph{Decoy Effects.} \changedin{1B}{1B-tables}{The various EW tables detail}{Tables~\ref{table:ew-decoy-ppl-effectiveness} gives} the specific effects or die roll modifiers PPLs produce. The following is a summary:
...
\paragraph{RWR Capabilities.} \changedin{1B}{1B-tables}{The RWR Table}{Table~\ref{table:ew-coverage}} lists ground unit radars (according to the frequency they use) and air radars (by modes) and indicates if that frequency or mode is detectable to each type of RWR.
...
\paragraph{Sighting Effects.} A RWR detection counts for the purposes of the \minus{1} modifier to the sighting roll for having a detection of the target. This modifier is applied only once regardless of how many different types of detections or lock-ons the searcher has for the target.
