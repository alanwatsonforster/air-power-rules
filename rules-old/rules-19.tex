...
\begin{advancedrules}
    ...
    \addedin{1C}{1C-apj-35-qa}{IFF status is declared during the aircraft decisions phase.}
    ...
    \addedin{1C}{1C-tsoh-errata}{
        \input{tables/table-internal-dds}
    }

    \subsection{Internal DDS} Internal DDS installations are identified on the ADC. Four types of internal DDS are available: A, B, C, D. \changedin{1C}{1B-tsoh-errata}{The internal DDS Table}{Table~\ref{table:ew-internal-dds}} shows the standard loads of decoys available for an internal DDS but typically they are capable of holding between sixteen and twenty clusters of decoys.

    \subsection{External DDS} External DDS pods are identified on the External Pod Table, which shows the possible loads of decoys available.

    \subsection{Types of Decoys} Three types of disposable decoys for DDS are available: flares (FL), chaff (CH), and jammers (MJ).

    \subsection{DDS Programs} An aircraft may use automatic programs which provide continuous protection by dispensing decoys throughout a game turn. A program is declared as ON or OFF during the Aircraft Decisions Phase. When on, the program will provide a LEVEL of protection of from 1 to 6 depending on its design. This is the aircraft's PPL (Program Protection Level). A PPL is in effect from the time the program is turned on until it is changed, turned off, or available decoys are exhausted. A PPL number represents both the level of protection and the number of decoys dispensed in a game-turn when the program is on.

    \subsection{DDS Program Design} A DDS program design is noted on paper in the following format (in PPL numbers):
    \begin{center}
    ...
    \subsection{Decoy Effects} \changedin{1B}{1B-tables}{The various EW tables detail}{Tables~\ref{table:ew-decoy-ppl-effectiveness} gives} the specific effects or die roll modifiers PPLs produce. The following is a summary:
    ...
    \addedin{2B}{2B-multiple-dds}{
        \subsection{Multiple DDS}
    ...
    \subsection{Sighting Effects} If the target used a DDS program with flares in the previous game turn, apply the PPL as a modifier to the sighting roll (see rule \ref{rule:sighting-aircraft-and-missiles}).
    ...
    \subsection{RWR Capabilities} \changedin{1B}{1B-tables}{The RWR Table}{Table~\ref{table:ew-coverage}} lists ground unit radars (according to the frequency they use) and air radars (by modes) and indicates if that frequency or mode is detectable to each type of RWR.

    \subsection{RWR Benefits Against Radar Equipped Units} All RWRs have the ability to indicate the relative direction of incoming radar strobes. Thus, if an aircraft with an RWR is searched for and/or locked onto by a detectable radar equipped enemy aircraft or ground unit, that aircraft receives a \minus{1} to any visual sighting rolls made against those radar equipped units.

    \subsection{RWR Benefits Against Missiles} If a SAM TTR, air radar illumination, or AHM active radar is detectable by the RWR, any missiles directed by those radars may be defensively engaged without being visually spotted.
    ...
    \subsection{Increased PPL Effectiveness} Due to the sophisticated logic interfaces of late 1980s and 1990s EW systems, DDS-C or DDS-D, when used with a RWR-C or RWR-D have increased PPL effectiveness as follows:

    \begin{itemize}
        \item When a PPL is in effect against radar systems or missiles which are detectable to RWR-C or RWR-D, the selected PPL level is increased by \plus{1} unless it was 0 to being with. Decoy use is not increased, just the effective PPL number.
    \end{itemize}

    \subsection{RWR Special Capabilities} RWR-A and RWR-B only indicate the general type of radar and frequency being used against an aircraft (i.e., CG SAM TTR, low freq.). RWR-C and RWR-D tells the crew the exact threat being employed against them (e.g., SA-11 SAM missile lock-on and launch). RWR-C and RWR-D can detect launches of CG and CW SAMs even when launched under OG.

    \subsection{RWRs Versus Hidden Units} If hidden initial ground forces are used, a player using radars must reveal to the player with the RWR-equipped aircraft the angle-off arc in which the radar energy was detected.

    If equipped with RWR-D, the radar player must also reveal the megahex in which the radar is located. The aircraft may then attempt to determine the exact hex on a die roll of 3 or less. If successful, the hidden unit is revealed and placed on the game map. If not found initially, the aircraft may (if the radar continues to operate) roll again in the SAM interaction phase of subsequent game-turns to locate the radar. A cumulative modifier of \minus{3} per game-turn after the turn of initial detection applies.

    \addedin{2B}{2B-sighting-and-other-detections}{
        \subsection{Sighting Effects} A RWR detection counts for the purposes of the \minus{1} modifier to the sighting roll for having a detection of the target. This modifier is applied only once regardless of how many different types of detections or lock-ons the searcher has for the target.

        \subsection{Advantage Category Effects} A RWR detection \emph{does not} count as a detection for the purposes of determining advantage category.
    }
    ...
    \addedin{1C}{1C-apj-27-qa}{Unless otherwise stated in the ADC, aircraft with internal BJMs and dedicated crew to run them (electronic warfare officers) have no frequency limits.}
    ...
    A barrage jammer has two modes: Noise and Stand-Off. The ability of an aircraft to switch between selected modes and jamming frequencies varies with the type of jammer and crew (pilot only or multi-crew) \changedin{1B}{1B-tables}{as detailed in the EW charts and EP tables}{is shown in Table~\ref{table:ew-bjm-flexibility}}.
    ...
    \begin{itemize}
        ...
        \item \itemparagraph{Stand-Off Mode.} Instead of flooding an arc with noise, the BJM can be focused on particular enemy radars in a concentrated jamming attack. A successfully jammed radar is blind and may not search, lock-on, or guide radar missiles. Stand-off jamming does not otherwise protect other aircraft. \changedin{1B}{1B-tables}{The EW tables}{Table~\ref{table:ew-bjm-capabilities}} indicate how many stand-off attacks can be made by a single jammer and the die roll required for success. A radar's ECCM acts as a modifier against the jamming roll.

    \end{itemize}

    \addedin{1C}{1C-apj-39-qa}{BJMs do not break locks once achieved.}

    \addedin{1C}{1C-apj-39-qa}{All internal BJMs may affect both air search and lock-on die roll attempts.}
    ...
    \addedin{1C}{1C-apj-39-qa}{All type A and B internal AJMs affect only air radar lock on attempt rolls. All type C and D internal AJMs affect both search and lock-on attempt rolls.}
    ...
    Refer to \changedin{1B}{1B-tables}{the RWR/DJM coverage table}{Table~\ref{table:ew-coverage}} to see if a DJM is effective against particular radars. If so, take the DJM's numerical self-protection rating and subtract the radar's ECCM rating from it.  If the result is positive, this is the number or less that must be rolled on one die to break the lock-on. DJMs also provide modifiers to radar guided missile attacks as detailed in \changedin{1B}{1B-tables}{the EW tables}{Table~\ref{table:missile-attack-modifiers}}. If an aircraft has both podded and internal DJMs, only the most effective one in the given frequency is used.

    \addedin{1C}{1C-apj-39-qa}{All internal DJM types may attempt to break any air radar lock-ons.}

    \section{Jamming Cell Formations}

    \subsection{Jamming Cells} Two, three or four aircraft with identical BJMs, AJMs, or DJMs may fly in a “Jamming Cell” formation.
    \changedin{1C}{1C-apj-23-errata}{
        Any time aircraft with identical jammers are positioned as illustrated in \changedin{1B}{1B-figures}{the jamming cell diagram}{Figure~\ref{figure:jamming-cell-formations}}, the effective rating of their jammers is increased as indicated in the diagrams.
        \addedin{1C}{1C-tsoh-errata}{
            The benefit is \plus{1} to the jammer effectiveness number.
        }
    }{
        To be in a valid Jamming Cell formation, aircraft must be in adjacent hexes (as depicted in Figure~\ref{figure:jamming-cell-formations}), with no more than 30 degrees difference in facing and within 1 altitude level of an adjacent aircraft. The maximum size of a jamming cell is 4 aircraft. Each aircraft within those parameters has its ECM jammer rating increased by 1.
    }

    \notein{1C}{AWF: the TSOH errata states, “All aircraft in a jamming cell must be \plus{} or \minus{} one altitude level for increased jammer effectiveness.” This is included in the errata above.}
    ...
    \addedin{2B}{2B-infrared-jammers}{
        ...
        Integrated infrared jammers are noted on the ADC. Infrared jammer pods are listed in External Stores Tables, and may be mounted on any weapon station able to carry an ECM pod (EP).
    }
