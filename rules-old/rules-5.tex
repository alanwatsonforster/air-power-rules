...
\subsection{Using The Log Sheet}
...
\subsection{Horizontal Flight Points}
...
\addedin{2B}{2B-expending-fps}{
    \subsection{Expending Flight Points}
    Some game activities require that an aircraft expend a certain number of FPs. These include changing facing, preparing to execute a maneuvers, recovery, aiming, and tracking. For these purposes, an FP is considered to have been expended as soon as the aircraft begins the movement associated with that FP.
}
...
\addedin{2B}{2B-fractions}{The number of FPs that must be expended is often given as a fraction of the speed of the aircraft or the number of FPs available to it in the current game turn.
    In many cases, multiply by the fraction and round as directed. However, Table~\ref{table:fractions} is used to determine:
    \begin{itemize}
        \item the number of HFPs and VFPs required by a given flight type;
        \item SSGT requirements;
        \item aiming requirements and modifiers; and
        \item the reduction in engine thrust at high altitude.
    \end{itemize}
    The values in the table for 1/3 follow the rule \emph{round to nearest but retain halves}. That is, round down if the fraction is less than 0.5; round up if the fraction is more than 0.5; and do not round if the fraction is 0.5. The values in the table for 2/3 are the whole minus the value for 1/3.
...
\addedin{1C}{1C-apj-36-errata}{\subsection{Speed Requirements Split Between Game Turns} Some game activities, like ground-attack aiming, are measured in fractions of an aircraft's speed. If such a period extends between two game-turns, the highest speed in either game-turn is used to compute the completion requirements in the second turn.}

\addedin{2B}{2B-fp-stages}{
    \subsection{FP Stages}
    \label{rule:fp-stages}

    The activities around the expenditure of each FP occur in stages in the following order:
    \begin{enumerate}

            \itemaddedin{3A}{3A-aborting-damage-control}{Aborting Damage Control Stage. If the moving aircraft is performing damage control, it may declare that it is aborting this. If does not, then for this FP it must fly within the restrictions applicable to damage control and in particular may not attack (see rule \ref{rule:progressive-damage}). If it does, it is no longer bound by these restrictions.}

        \item Preemption Stage. Any other aircraft threatened by the moving aircraft can declare and execute a preemption.

        \item SSGT Tracking Stage. The moving aircraft may declare that it will start SSGT of a specified target aircraft.
            FPs subsquently expended by the moving aircraft then count towards the tracking requirement.

        \item AAA Tracking Stage. If the moving aircraft is within tracking range of any AAA units, any of these may declare that they are tracking the aircraft. FPs subsquently expended by the moving aircraft then count towards the tracking requirement.

        \item Aiming Stage. The moving aircraft may declare that it will start aiming at a specified ground unit.
            FPs subsquently expended by the moving aircraft then count towards the aiming requirement.

        \item Movement Stage.

            \begin{enumerate}
                \item The moving aircraft can declare a turn or a maneuver or stop one. Declaring a turn implicitly sets the bank.

                \item The moving aircraft may expend an FP to change position (map location and altitude). For the purposes of expending FPs to fulfill turn requirements, preparations for maneuvers, recovery, aiming, and tracking, the FP is considered to have been expended as soon as the aircraft starts to move.

                \item If the moving aircraft is turning and has expended FPs at least equal to the turn requirement (including the current one), it may complete the turn and change facing.

                \item If the moving aircraft is not turning, it may set its bank according to the rules.
...
\begin{advancedrules}
    ...
    \subsection{After Climbing or Diving}
    ...
    \subsection{Formation Types} Two types of formations are possible: Close and Tactical. Close formations are designated by stacking aircraft in the same hex. Tactical formations exist whenever certain spacing conditions are met.

    \subsection{Formation Size} Formations can be of the following sizes:
    ...
    \subsection{Forming Close Formations} A close formation may be formed prior to the beginning of a scenario (during set up), or in the Aircraft Admin Phase of a game turn whenever two, three, or four friendly aircraft end up in the same position with the same exact facing, speed and altitude.

    \subsection{Splitting Close Formations} A close formation may split up by declaring the intent to detach airplanes from the stack when it moves. Aircraft which detach are left in the starting hex while the rest of the formation executes its flight. The detached aircraft are then flown.
    ...
    \begin{itemize}
        ...
        \item and the wingmen’s' facings are no more than \degrees{60} different from the leader's facing (left or right),
