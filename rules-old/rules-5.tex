...
\addedin{2R}{2R-expending-fps}{
\paragraph{Expending Flight Points.}
Some game activities require that an aircraft expend a certain number of FPs. These include changing facing, preparing to execute a maneuvers, recovery, aiming, and tracking. For these purposes, an FP is considered to have been expended as soon as the aircraft finishes the movement associated with that FP and before any immediately subsequent turn completions or attacks.
}
...
\end{itemize}
With the exception of the special case for 0.5, the values in the table follow the rule \emph{round to nearest but retain halves}. That is, round down if the fraction is less than 0.5; round up if the fraction is more than 0.5; and do not round if the fraction is 0.5.
...
\item Expenditure Stage. The moving aircraft may expend an FP to change position (map location and altitude) either by normal movement or by executing a maneuver. Some maneuvers also change facing. For the purposes turn requirement, preparation for maneuvers, recovery, aiming, tracking, the FP is considered to have been expended at the end of this stage. 

\item Bank Stage. If the moving aircraft is not turning, it may set its bank according to the rules.

\item Turn Completion Stage. If the moving aircraft is turning and has expended FPs at least equal to the turn requirement, it may complete the turn and change facing.
