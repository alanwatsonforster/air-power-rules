...
Some game activities require that an aircraft expend a certain number of FPs. These include changing facing, preparing to execute a maneuvers, recovery, aiming, and tracking. For these purposes, an FP is considered to have been expended as soon as the aircraft begins the movement associated with that FP and before any immediately subsequent turn completions or attacks.
}
...
\item Movement Stage. The moving aircraft may expend an FP to change position (map location and altitude) either by normal movement or by executing a maneuver. Some maneuvers also change facing. If the moving aircraft is turning and has expended FPs at least equal to the turn requirement (including the current one), it may complete the turn and change facing. If the moving aircraft is not turning, it may set its bank according to the rules. 

For the purposes of expending FPs to fulfill turn requirements, preparations for maneuvers, recovery, aiming, and tracking, the FP is considered to have been expended as soon as the aircraft starts to move.
