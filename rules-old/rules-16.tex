...
\paragraph{Die Roll Modifiers.} Detection die roll modifiers exist for electronic jamming, the presence of chaff and mini-jammer programs, stealth technology, crew quality, and/or aircraft size. These are summarized \changedin{1B}{1B-tables}{on the Search Modifiers Table}{in Tables~\ref{table:radar-search-modifiers} and \ref{table:ew-radar-modifiers}}.
...
\paragraph{Procedure.} Roll the die. If the result after applying any modifiers is less than or equal to the lock-on number listed in the radar section of the ADC, the target is locked-onto. \changedin{1C}{1C-apj-25-qa}{

\paragraph{Die Roll Modifiers.} The same modifiers that apply to search rolls, apply to lock-on rolls.}{The lock-on modifiers are given in \changedin{1B}{1B-tables}{the play aids}{in Table~\ref{table:radar-lock-on-modifiers}}.}
...
\begin{advancedrules}
    ...
    \paragraph{Auto-Track Radar Mode.}\label{rule:auto-track-mode} Auto-Track Radar mode allows an aircraft to automatically detect and then lock-on a target. Only aircraft with Auto-Track Technology may use this mode. \addedin{1B}{1B-tables}{Table~\ref{table:radar-auto-track-mode} summarizes auto-track mode.}
