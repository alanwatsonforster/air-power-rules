...
If more than one aircraft, including friendlies, are in an UNCAGED FOV, the firing player chooses the intended target and rolls a die. If the target is the nearest aircraft, the lock-up succeeds on a roll of 8 or less. If other aircraft in the FOV are equally near, or if the target is not the closest, then the die roll is modified by a cumulative $+1$ for every closer or equally near aircraft. If successful, missiles may be fired normally. If the lock-up attempt fails, missiles may not be launched. \addedin{2B}{2B-uncaged-lock-up}{The roll to determine whether a lock-up succeeds is made after declaring missile launches. If all declared launches fail, as usual one additional launch may be attempted.}
...
\begin{enumerate}
    ...
    \item[(b)] The flight slope since launch or since the last climbing VFP (whichever is later) is $-1$ or steeper (more negative).
\end{enumerate}
