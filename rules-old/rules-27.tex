...
\subsection{Launching CG Weapons} The aircraft must steep dive on a LOA to the target and be within the rocket's minimum and maximum launch ranges (counting altitude). The rocket may be launched after any expenditure of FP while wings level (not turning or maneuvering). Only one guided rocket may be launched in an attack.

Note: Aiming need not be completed but if not, the \plus{3} attack modifier applies and all bombsights degrade to manual. A die roll is required for successful launch.

\subsection{CG Rocket Flight} Upon launch, a CG rocket immediately flies its full speed along the LOA toward the target. The rocket and aircraft alternate expending FPs as if proportionally moving with a missile (rocket moves first). Shoot-outs are resolved normally if need be. In some cases, the aircraft will run out of FPs early since it fired in mid-flight. On the first turn of flight, the rocket's start speed is its base speed plus that of the aircraft. If the rocket needs additional turns of flight to reach the target, \changedin{1C}{1C-apj-27-qa}{it begins to attenuate speed}{its speed is changed according to the rules for missile sustainers and speed attenuation} just like an air to air missile.

\subsection{CG Rocket Guidance} As long as the rocket is moving toward the target, the guiding aircraft must be in steep diving flight and must remain on the line of approach. If the aircraft does not meet either of these criteria before the rocket hits, the rocket will miss automatically. Note that guiding aircraft move before other free aircraft.

\subsection{CG Rocket Attack} Attacks are resolved normally using the weapon's attack strength. Only the primary target can ever be hit. \changedin{1C}{1C-apj-23-errata}{Against vehicular units, the highest result that can be achieved is a D for weapons of 1000 lbs or less and 2D for larger weapons.}{The maximum damage allowed against vehicle units effected by RCG and ACG weapons is D for weapons of less than 1000 lbs and 2D for 1000 lbs or larger weapons.} For guided rocket attacks, only the aircraft damage, bombsight modifiers are used. Tracking time and rocket attack slant range modifiers do not apply.
...
\subsection{Laser Designators} Ground FACs and aircraft with either laser designator technology or equipped with laser pods (LP) are allowed to place laser spots on targets. A ground FAC may place a spot on any enemy target in its line of sight within six hexes. Aircraft designators vary in capability depending on type. There are three types:

\begin{itemize}

\item\itemparagraph{Type-A:} These may place a laser spot anywhere in the aircraft's \arcplus{180} arcs if operated by the pilot, or within the \arcplus{120} arcs if used by the weapons officer. The maximum range a laser spot may be placed from the aircraft is 18 hexes (counting altitude).
...
\subsection{Laser Spot Trackers} Aircraft with LSTs can utilize laser spots for accurate aiming and are thus allowed the die roll modifier shown on the attack table when laser spot marks their target whether they use conventional or laser guided weapons.

%Aircraft do not require a LST to use a laser guided weapon.

\subsection{Joint Attacks} A designator equipped aircraft is allowed to place laser spots for other aircraft to drop bombs on. This is called a joint attack and is declared in the aircraft decision's phase by identifying the participating aircraft.
...
\subsection{Designator Limitations} Engaged, stalled and departed aircraft may not designate. Using a designator imposes the following maneuver restrictions on free aircraft for as long as they are maintaining a spot:
...
\subsection{Laser Guided Bombs} Laser guided bombs (BG) are aimed and dropped just like BB class weapons including the use of regularly allowed modifiers except:
...
\item If the dropping aircraft is not also the designator and has laser spot tracker technology or a pod allowing it that, an additional \minus{1} to the attack roll is allowed.

\item If no laser spot is available, the weapons may be dropped as regular BB class weapons using the BB attack strengths.

\item \addedin{1C}{1C-original-play-aids}{Laser guided bombs only get the \minus{4} to the attack roll if laser spot on target at time of inpact. }If the laser spot is lost after the weapons are dropped under guidance, they revert to regular BB class weapons but incur the \plus{3} modifier for not being aimed.
...
A laser guided rocket is flown exactly as a CG rocket (including shoot-out procedures) except that the aircraft is free to maneuver after launch as no guidance inputs are required unless it is designating as well. If aiming is not completed, the \plus{3} modifier applies and the bombsight is considered manual. The following also apply:
...
\item If the firing aircraft is not also the designator and has laser spot tracker technology or a pod allowing it that, an additional \minus{1} to the attack roll is allowed.

\item RGs may be fired even if no laser spot exists. In such a case, they are treated as \changedin{1C}{1C-apj-23-errata}{unguided heavy rockets and use the RKs}{unguided rockets using their regular} attack value and all the normal unguided air to ground rocket modifiers.

\item If a laser spot is lost before RGs already in flight reach the target, they revert to being unguided rockets as above but with a \plus{3} (no aiming) modifier and the bombsight modifier is manual.
