...
If the proportion includes leftover FPs, for example, say the \changedin{3A}{3A-fps-for-proportional-moves}{missile's speed was 17.0 instead of 15.0}{has 17 FPs instead of 15}. In this case five goes into seventeen three times with two left over. The missile would still move three for one except the two extra points are added on one at a time in the first two segments of movement that turn. That is, \changedin{2A}{2A-missile-flight}{the aircraft would move one, then the missile four ($3+1$), then the aircraft one and missile four again}{the missile would move four FPs, then the aircraft one, then the missile four, and the the aircraft one again}. After that, the missile reverts to its three to one proportion.
...
If a free aircraft under attack by an IRM selects idle power and fails the roll to obtain the corresponding modifier, it uses the modifier for normal power (i.e., $+0$). A free aircraft under attack by an IRM can select normal power and automatically gain the corresponding modifier. If an aircraft was using idle power at the moment it became the target of an IRM and continues to use idle power until the missile attacks, then it gains the $+1$ modifier for idle power whether or not it engages the missile.
}
