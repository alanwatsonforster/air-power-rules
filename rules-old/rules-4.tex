...
\begin{enumerate}[resume]
    ...
    \item \itemparagraph{Configuration Points Limits.}
        ...
        The sum of the load an aircraft is carrying at any instant, in load points\deletedin{3W}{3A-load-rounding}{ (rounded down)}, is compared to the limits given for the three possible configurations (CL = clean, 1/2 = half-dirty, DT = Dirty). \changedin{3W}{3A-load-rounding}{Where the sum falls within those limits defines the current configuration of the aircraft.}{If the sum satisfies the limits for CL or for DT, the aircraft has the corresponding configuration. Otherwise, the aircraft has 1/2 configuration.} See advanced rule \ref{rule:aircraft-configuration}.
...
\begin{advancedrules}
    ...
    Aircraft can be flown in one of three possible game configurations: Clean (CL), Half Dirty (1/2), and Dirty (DT). The actual configuration is determined by the \changedin{3W}{3A-load-rounding}{amount of}{total} load points carried on the aircraft's weapons stations.  Each Aircraft Data Card indicates the point limits which establish Clean, Half, and Dirty. \deletedin{3W}{3A-load-rounding}{When adding up the load, first total all points then round any fractions \changedin{1C}{1C-apj-23-errata}{off}{down}.}
