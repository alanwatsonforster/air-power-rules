...
This chapter discusses the Aircraft Data Cards (ADCs) and the information presented on them. Each aircraft used in play has its flight and combat capabilities fully defined in game terms in its ADC. A sample ADC is shown \changedin{1B}{1B-figures}{on the following pages}{in Figure~\ref{figure:adc}}.  It is divided into two major panels: The Flight Characteristics panel and the Combat Characteristics panel.

\addedin{3A}{3A-updating-adcs}{
    There have been three editions of ADCs. These rules use the third editions. The procedures for updating earlier editions are given in Appendix~\ref{appendix:updating-adcs}.
}
...
\begin{enumerate}
    ...
    \item \itemparagraph{Aircraft Type.}
        The upper right corner contains the name most commonly used to identify that type of aircraft. \deletedin{3W}{3W-adc}{It also shows the number of crew positions on the aircraft.}

    \item \itemparagraph{Three-View.}
        ...
        \itemaddedin{3W}{3W-adc}{\itemparagraph{Crew.} The upper right corner shows the crew positions.}
...
\begin{enumerate}[resume]
    ...
    \item \itemparagraph{Internal Gun Data.}
        The Internal Gun section shows the characteristics of the aircraft's internal guns if any. The data is fully explained in the gun combat rules (rule \ref{rule:air-to-air-gun-and-rocket-combat}
        ).
    ...
    \item \itemparagraph{Configuration Points Limits.}
        ...
        The sum of the load an aircraft is carrying at any instant, in load points\deletedin{3A}{3A-load-rounding}{ (rounded down)}, is compared to the limits given for the three possible configurations (CL = clean, 1/2 = half-dirty, DT = Dirty). \changedin{3A}{3A-load-rounding}{Where the sum falls within those limits defines the current configuration of the aircraft.}{If the sum satisfies the limits for CL or for DT, the aircraft has the corresponding configuration. Otherwise, the aircraft has 1/2 configuration.} See advanced rule \ref{rule:aircraft-configuration}.
...
\begin{advancedrules}
    ...
    Aircraft can be flown in one of three possible game configurations: Clean (CL), Half Dirty (1/2), and Dirty (DT). The actual configuration is determined by the \changedin{3A}{3A-load-rounding}{amount of}{total} load points carried on the aircraft's weapons stations.  Each Aircraft Data Card indicates the point limits which establish Clean, Half, and Dirty. \deletedin{3A}{3A-load-rounding}{When adding up the load, first total all points then round any fractions \changedin{1C}{1C-apj-23-errata}{off}{down}.}
    ...
    \subsection{Changes in Configuration}
    ...
    \begin{itemize}
        ...
        \item Engine Power Available. Configuration changes can only occur after the power setting is selected, so no increase in Accel is available until the following turn.
            ...
            \itemaddedin{2B}{AWF}{Cruise Speed. If idle or normal power is selected, the cruise speed is modified according to the configuration at the start of a game turn.}
    ...
    \addedin{1C}{1C-apj-23-errata}{\subsection{Over-Loading Stations}
    ...
    \addedin{1C}{1C-apj-23-errata}{\subsection{Symmetric Loading Requirements} When picking loads for aircraft, the initial weight carried on one wing must be within 20\% of that carried on the other wind unless the total weight on each wing is 10\% or less of the aircraft's load limit.
    }

    Configuration Example: An aircraft with the following configuration limits: CL = 0-8, 1/2 = 9-14, and DT = \plusafter{15}; that is loaded with two Triple Racks (TRs) and six Mk.82 500lb HE bombs, is carrying 11 load points and would be 1/2 configured. The TRs are worth one load point each and the bombs are 1.5 points each. If a 1200L fuel tank were added (4 load points), the aircraft would be considered DT configured (15 points total).
