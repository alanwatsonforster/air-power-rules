...
Aircraft can be flown in one of three possible game configurations: Clean (CL), Half Dirty (1/2), and Dirty (DT). The actual configuration is determined by the \changedin{3X}{3X-load-rounding}{amount of}{total} load points carried on the aircraft's weapons stations.  Each Aircraft Data Card indicates the point limits which establish Clean, Half, and Dirty. \deletedin{3X}{3X-load-rounding}{When adding up the load, first total all points then round any fractions \changedin{1B}{1B-apj-23-errata}{off}{down}.}
