...
\begin{itemize}
    ...
    \item it ends its move with the target outside the missile's own \arcrange{180}{+} arc.
...
\begin{itemize}
    ...
    \item have a target inside his normal radar arc, but not past his \arcrange{150}{+} arc.
...
\begin{itemize}
    ...
    \item it ends its move with the target outside the missile's own \arcrange{150}{+} arcs.
...
\begin{itemize}
    ...
    \item MCG Capable AHM: An MCG missile need only keep the firer in its 90 degree or less arc (in order to receive guidance signals in its rear antenna). The radar vertical limits table does not apply in this case. While in mid-course guidance, the AHM also does not have to keep the target in its \arcrange{150}{+} arc.

\end{itemize}

\paragraph{Terminal Active Homing Phase.} At the instant an AHM moves into active homing range and the target is in its \arcrange{150}{+} arc, its own radar takes over. The firer is freed from the requirement to maintain lock-on and/or illuminate the target.  When active, an AHM must end each proportional move and/or game turn no further away than when it started and with the target in its \arcrange{150}{+} arc, otherwise it is removed from play.
