...
\paragraph{BRM Guidance Requirements.} To successfully guide a BRM, the firing aircraft must ILLUMINATE the target (rule \ref{rule:target-illumination}) and maintain a lock-on until the missile hits, misses or is removed from play.
...
\paragraph{Chaff Decoys.} Radar guided missiles are vulnerable to chaff just like IRMs are vulnerable to flares. Chaff may be dispensed manually or through a DDS program as described in rule \ref{rule:irm-countermeasures}. Also see rule \ref{rule:electronic-warfare}.
