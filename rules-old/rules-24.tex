...
%Clarification:  Chance hits of Heavy AAA in a plotted fire zone have an attack rating of “2”.  Regular hits achieved when plotted fire is revealed and resolved use the regular die roll procedure to determine attack rating.
...
Maneuvering aircraft are more difficult targets for AAA guns. A good strike pilot will develop his own pattern of jinks to fool enemy gunners. \addedin{1B}{1B-tables}{The rules for jinking are described here and summarized in Table \ref{table:jinking}.}
...
Start with the nearest aircraft to which the unit has a line of sight and which is sighted (if the AAA unit is equipped with an FCR, the aircraft does not have to be sighted). Roll the die once and consult \changedin{1B}{1B-tables}{the plotted fire direction diagrams}{Figure \ref{figure:random-aaa}} to determine the direction the plotted fire hex will be from the aircraft. A zero indicates the plotted hex is the aircraft's hex.
