...
\subsection{AAA Damage} Damage to aircraft is determined using the Damage tables as per Rule \ref{rule:aircraft-damage-resolution}.
...
\subsection{Tracking} To track a target, the AAA unit must have a line of sight to it, and the target must be within one and a half times the AAA unit's maximum effective range. Tracking begins when the AAA unit declares it is doing so. Tracking is considered complete for manually aimed guns the moment a tracked aircraft expends FPs equal to 1/2 or more of its speed (round fractions up) in the AAA unit's line of sight. Tracking is considered complete for \changedin{2W}{2W-radar-directed}{radar directed guns}{radar-directed guns (those using an add-on FCR or an integral “W” FCR)} the moment FPs equal to 1/3 the target's speed (round fractions up) are expended.
...
\addedin{2B}{2B-expending-fps}{
    For example, a light AAA unit has a line of sight to an aircraft that is within its tracking range. The aircraft has a speed of 4.5. The AAA unit declares that is is tracking the aircraft. If the AAA unit were manually controlled, it would be able to attack after the aircraft had expended three FPs and provided the other conditions were met. If the AAA unit were radar-directed, it would be able to attack after the aircraft had expended two FPs.
}
...
\subsection{Switching targets} AAA units planning to use aimed fire which track but do not shoot at one aircraft in a game turn may switch to and track subsequent aircraft that move. Each switch within a game turn however, carries a cumulative \plus{1} modifier to the aimed fire die roll.
...
\subsection{Plotted Fire Procedure} During the AAA Interaction Phase, a target hex and altitude is secretly noted for each AAA unit using plotted fire.
...
\subsection{Weather Effects (Advanced Rule \ref{rule:night-and-weather}).} Add on FCRs are all weather capable and allow the AAA unit to track and fire on enemy aircraft at night and in adverse weather even if not visually sighted.
...
\label{rule:aaa-and-aircraft-sighting}

\subsection{Sighting Restrictions} AAA units and small arms capable ground units are assumed to automatically sight aircraft and may begin tracking them at any point in the Flight Phase in which the aircraft becomes or is visible to them based on line of sight restrictions and the following limitations:
...
\begin{advancedrules}
    ...
    \subsection{Jinking Parameters} To be considered jinking, an aircraft must meet one of the following parameters:
    ...
    \subsection{Jinking Effect} A jinking aircraft which is hit by AIMED or PLOTTED AAA fire is allowed an immediate die roll to negate the hit; meaning the gunners actually missed. Roll a die before any attack rating or damage is determined.

    If the roll is a 6 or more against manually directed guns, or 8 or more against \changedin{2W}{2W-radar-directed}{radar directed guns}{radar-directed guns (those using an add-on FCR or an integral “W” FCR)}, the hit is nullified and becomes a miss. Consider it a paint scorching “Close, but no cigar” shot. The aircraft size modifier is applied to this roll. Jinking aircraft taking a Chance hit from a barrage or plotted fire zone, do not get a nullifying die roll.
    ...
    Some scenarios call for AAA to be fired randomly, or to be played solitaire. In these cases, AAA units fire only when determined by a die roll \changedin{1B}{1B-tables}{on the Random AAA Fire Tables as described below}{as described below and in Table \ref{table:random-aaa}}.

    \subsection{Aimed Fire} Light and Medium AAA will always use random aimed fire in solitaire play.

    Whenever an aircraft expends an FP in the range of an unfired AAA unit, roll one die. Check the result with \changedin{1B}{1B-tables}{the Random Fire Table}{Table \ref{table:random-aaa}} and if a shot is indicated, resolve it via the normal AAA rules. The probability of a unit firing increases at closer ranges. Remember to check for each AAA unit separately and begin with the unit closest to the moving aircraft, then moving out to the next closest and so on. As long as AAA units remain to be fired, check for each subsequent aircraft that moves in range.

    \subsection{Barrage Fire} Any non-AAA units capable of barrage fire are considered to use it all times.

    \subsection{Plotted Fire} Heavy AAA will always use plotted fire. The target altitude and hex are determined randomly as follows in the Ground Unit Interaction Phase:
