%!TEX root = ../rules-working.tex
%LTeX: enabled=false

\silentlychangedin{1B}{1B-figures}{

\begin{onecolumnfigure}
\caption{AIR-2 Genie Scatter}
\medskip
\includegraphics[width=0.5\linewidth]{figures/aids-genie-scatter.pdf}
\end{onecolumnfigure}

}{
\begin{twocolumnfigure}

\begin{fitheight}{5.2\standardhexwidth}
\begin{tikzpicture}
    \setfiguresize{-2.6}{-2.6}{+2.6}{+2.6}
    \begin{scope}
        \drawevenhexgrid{-2}{-2}{5}{5}
        \begin{scope}[dashed,very thick, ->,every node/.style={solid,minimum size=0.5cm, draw=black!50, fill=white}]
            \draw (0,0) --   (90:1.5) node [anchor=270] {1};
            \draw (0,0) --   (60:1.5) node [anchor=240] {2};
            \draw (0,0) --   (30:1.5) node [anchor=210] {3};
            \draw (0,0) --    (0:1.5) node [anchor=180] {4};
            \draw (0,0) --  (330:1.5) node [anchor=150] {5};
            \draw (0,0) --  (270:1.5) node [anchor=90 ] {6};
            \draw (0,0) --  (210:1.5) node [anchor=60 ] {7};
            \draw (0,0) --  (180:1.5) node [anchor=0  ] {8};
            \draw (0,0) --  (150:1.5) node [anchor=330] {9};
            \draw (0,0) --  (120:1.5) node [anchor=300] {10};
        \end{scope}
        \silentlychangedin{2B}{2B-genie-scatter-counter}{
            \drawaircraftcounter[0]{+0.00}{+0.00}{90}{F-105}{}{}
        }{
            \drawarrowcounter{0}{0}{90}
        }
     \end{scope}
\end{tikzpicture}
\end{fitheight}
\hfil
\begin{fitheight}{5.2\standardhexwidth}
\begin{tikzpicture}
    \setfiguresize{-2.6}{-2.6}{+2.6}{+2.6}
    \begin{scope}[rotate=90]
        \drawevenhexgrid{-2}{-2}{5}{5}
        \changedin{2B}{2B-genie-scatter-figure}{
            \node at (0,0) [draw,fill=white] {Diagram Missing in Original};
        }{
            \begin{scope}[dashed,very thick,->,every node/.style={solid,minimum size=0.5cm, draw=black!50, fill=white}]
                \draw (0,0) --   (0:1.5) node [anchor=270] {1};
                \draw (0,0) -- (330:1.5) node [anchor=240] {2};
                \draw (0,0) -- (300:1.5) node [anchor=210] {3};
                \draw (0,0) -- (270:1.5) node [anchor=180] {4};
                \draw (0,0) -- (240:1.5) node [anchor=150] {5};
                \draw (0,0) -- (180:1.5) node [anchor=90 ] {6};
                \draw (0,0) -- (120:1.5) node [anchor=30 ] {7};
                \draw (0,0) --  (90:1.5) node [anchor=0  ] {8};
                \draw (0,0) --  (60:1.5) node [anchor=330] {9};
                \draw (0,0) --  (30:1.5) node [anchor=300] {10};
            \end{scope}
            \silentlychangedin{2B}{2B-genie-scatter-counter}{
                \drawaircraftcounter[90]{+0.00}{+0.00}{0}{F-105}{}{}
            }{
                \drawarrowcounter{0}{0}{0}
            }
        }
    \end{scope}
\end{tikzpicture}
\end{fitheight}
\hfil
\begin{fitheight}{5.2\standardhexwidth}
\begin{tikzpicture}
    \setfiguresize{-2.6}{-2.6}{+2.6}{+2.6}
    \begin{scope}[rotate=90]
        \drawoddhexgrid{-2}{-1.5}{5}{5}
        \begin{scope}[dashed,very thick,->,every node/.style={solid,minimum size=0.5cm, draw=black!50, fill=white}]
            \draw (0,0) --    (0:1.7) node [anchor=270] {1};
            \draw (0:1.000*\hexxfactor) -- (1.7,-0.5) node [anchor=240] {2};
            \draw (0:0.333*\hexxfactor) --  +(300:1.2) node [anchor=210] {3};
            \draw (0,0) --  (270:1.5) node [anchor=180] {4};
            \draw (180:0.333*\hexxfactor) --  +(240:1.2) node [anchor=150] {5};
            \draw (0,0) --  (180:1.7) node [anchor=90] {6};
            \draw (180:0.333*\hexxfactor) --  +(120:1.2) node [anchor=30 ] {7};
            \draw (0,0) --   (90:1.5) node [anchor=0  ] {8};
            \draw (0:0.333*\hexxfactor) --  +(60:1.2) node [anchor=330] {9};
            \draw (0:1*\hexxfactor) -- (1.7,+0.5) node [anchor=300] {10};
        \end{scope}
        \silentlychangedin{2B}{2B-genie-scatter-counter}{
            \drawaircraftcounter[90]{+0.00}{+0.00}{0}{F-105}{}{}
        }{
            \drawarrowcounter{0}{0}{0}
        }
    \end{scope}
\end{tikzpicture}
\end{fitheight}

\figurecaption[The counter represents the position and facing of the Genie at the end of its move. A first die roll determines the scatter direction. A second die roll, halved and rounded down, determines the scatter distance in hexes.]{figure:genie-scatter}{AIR-2 Genie Scatter.}

\end{twocolumnfigure}
}
