...
\begin{itemize}
    ...
    \itemtag{2B-damage-control-restrictions} The only climbing flight allowed during damage control is a SC without AB. The only diving flight allowed during damage control is a SD without AB and at most 2 VFPs
...
Version 3A contains unauthoritative major changes.
...
\begin{itemize}
    ...
    \itemtag{3A-missile-arc} If the missile is on the border between two arcs, then the case is resolved first by using the arc into which the faster element is moving and second by choosing the wider arc (the one closer to the \arc{120} arc).
