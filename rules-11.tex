...
\begin{itemize}
    ...
    \item Whether target is smoking, ejecting flares, or in various weather conditions.
\end{itemize}
...
\paragraph{Detection.} IRSTS detects heat based on fuel consumption: Detection range for Type A and B equals four times the fuel used number (based on the Fuel column of the Power Chart for the current power setting). For Type C, detection range equals six times the fuel used.  The range is doubled if the IRSTS is in the target's 30° arc or less. The target is detected if it is in detection range.
...
\paragraph{Ground FAC.} A ground FAC is a vehicle-mounted observer on the ground. A Ground FAC automatically sees any enemy ground units (camou-flaged or not) within six hexes provided LOS is not blocked. It sees units in adjacent hexes even if LOS is blocked.
