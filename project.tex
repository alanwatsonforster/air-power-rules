...
\begin{itemize}
    ...
    \itemtag{2B-unloaded-dives} The unloaded dive rules in version 2A gives no advantage over a steep dive. I have changed the rule so that all FPs are HFPs, one altitude level is lost if half of the FPs are spent unloaded, and two if all are spent unloaded. This gives one additional HFP over an otherwise similar steep dive.    
