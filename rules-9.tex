...
\begin{itemize}
    ...
    \itemaddedin{1B}{JDW: TSOH errata}{\itemparagraph{Vertical Attack.} Climbing or diving aircraft firing at targets in the same hex at the same or at different altitudes may be subject to Vertical Attack Modifiers (see 9.2 below).}
...
\begin{itemize}
    ...
    \item An aircraft \addedin{1B}{JDW: TSOH errata}{in level flight} may fire at a target in another hex only if it is at the same altitude level or at an adjacent altitude level.
...
\addedin{1B}{JDW: TSOH errata}{The recovery period can also be used to reduce gunsight turning modifiers (i.e., an aircraft that uses the BT turn rate for an initial facing then expends at least half its FPs turning at the TT rate before firing, in this case TT gunsight mods, if any, would apply not the BT). Recovery period considerations also apply to air-to-air rocketry, missile launch-G modifiers, strafing, and Aiming for or making ground attacks following ET turns.}
...
\begin{itemize}
    ...
    \itemaddedin{1B}{JDW: TSOH errata}{Pilot Quality}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \changedin{1B}{JDW: TSOH errata}{One small rocket pod is equal to one air-to-air rocket factor.}{A small rocket pod, with 7 or fewer rockets, is equal to 1/2 an air-to-air rocket factor.}
    \item \changedin{1B}{JDW: TSOH errata}{One medium or large rocket pod is equal to two air-to-air rocket factors.}{A medium rocket pod, with 19 or fewer rockets, is equal to one air-to-air rocket factor.}
    \itemaddedin{1B}{JDW: TSOH errata}{A large rocket pod, with 20 or more rockets, is equal to two air-to-air rocket factors.}
\end{itemize}
...
\paragraph{Steady State Gunsight Tracking (SSGT).} Gunsights are optimized for rear quarter attacks. Any aircraft attacking from the 60° or less angle-off arc may track its target and achieve improved probabilities for hits. \changedin{1B}{JDW: TSOH errata}{The firing aircraft must expend FPs while on a tracking line (see Tracking diagram). For each 1/3 (round down) of the aircraft's speed (in full FPs) expended on a tracking line, modify the Roll To Hit by $-1$.}{The firing aircraft must expend FPs while on a tracking line with the target in its limited arc (see Tracking Diagram). For each 1/3 of the aircraft's speed expended on the tracking line (use the 1/3-2/3 conversion table), modify the Roll to Hit by $-1$.}  A maximum modifier of $-2$ ls allowed for SSGT; tracking may not begin until the firing aircraft is within six hexes range of the target.

\addedin{1B}{JDW: TSOH errata}{SSGT must be started anew each game turn. Taking a shot during a move does not cancel or stop the aircraft from doing SSGT.}

\addedin{1B}{JDW: TSOH errata}{A target that defensively pre-empts nullifies the attacker's SSGT up to that point. When the attacker resumes moving following a pre-emption, SSGT may be started anew.}
