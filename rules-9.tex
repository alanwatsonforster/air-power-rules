...
\paragraph{Head-On Gun Attacks.} If both the attacker and target directly face each other with the attacker on the \arc{180} line shown in Figures~\ref{figure:angle-off-facing-hex-corner}, \ref{figure:angle-off-facing-hex-side}, or \ref{figure:angle-off-on-hex-side}, the attack is a head-on attack. A target aircraft waiting to move (or that has already finished its move) may return fire in response to head-on attacks provided it does not exceed the two-shot limit per game turn and satisfies all of the normal prerequisites except the sighting requirement. An aircraft that has not flown yet may \emph{not} respond to a head-on attack if it is carrying or recovering from an ET turn or prepared for or executed a rolling maneuver on the last FP of the previous game turn. Gunsight modifiers apply if it is carrying a turn or recovering from a turn.
}
...
For example, a radar search field given as $\arcrange{60}{-}$ consists the \arc{30} and \arc{60} arcs and a blind arc given as $\arcrange{150}{+}$ consists of the \arc{150} and \arc{180} arcs. 
...
After using the ET turn rate, an aircraft may not carry out certain activities (including gun attacks and listed fully below) until it has completed a recovery period. The recovery period represents the time it takes for the crew or gunsight to recover from the high G forces in an ET turn.

A recovery period after an ET turn is completed when the aircraft has used consecutive FPs equal to half its speed (round down) while not turning at the ET rate and not preparing for or executing a rolling maneuver. The recovery period can start with the first FP expended after the completion of the turn. If the recovery period is interruped, it must start again.
...
Recovery periods (of all types) and their restrictions can extend into the subsequent game turn. If an aircraft has not recovered before the end of its move, it is prohibited from carrying our certain activities in the phases after the flight phase.

If an aircraft's speed changes between the game turns, the higher speed is used to compute the number of FPs in the recovert period.

For example, consider an aircraft that turned at the ET rate on the last FP of its move and then stops. It will not be able to launch missiles or use its radar in the subsequent air-to-air missile or air radar search and lock-on phases. If its new start speed is 7.0, its recovery period will be the first, second, and third FPs of the subsequent game turn. It will be able to conduct a gun attack after its fourth FP and will be able to participate normally in the subsequent air-to-air missile or air radar search and lock-on phases.

\paragraph{Recovery After Unloaded FPs.} Similarly, after an unloaded FP an aircraft may not carry out certain activities (listed below) until it has completed a recovery period. The recovery period represents the time it takes for the crew or gunsight to recover from weightlessness.

A recovery period after an unloaded FP is completed when the aircraft has used consecutive FPs equal to half its speed (round down) while not unloaded and not preparing for or executing a rolling maneuver. The recovery period includes the first FP expended after the unloaded FP. If the recovery period is interruped, it must start again.
...
A recovery period after an TT, HT, or BT turn is completed when the aircraft has used consecutive FPs equal to half its speed (round down) while not turning at the that rate and not preparing for or executing a rolling maneuver. The recovery period includes the first FP expended after the completion of the turn. If the recovery period is interruped, it must start again.
...
\paragraph{Recovery After Rolling Maneuvers.} In terms of gunsight modifiers, preparing for or executing a rolling maneuver is equivalent to turning at the BT turn rate.
