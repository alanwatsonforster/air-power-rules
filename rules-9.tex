...
\paragraph{Die Roll Modifiers.} The roll to hit is modified by a variety of circumstances. The possible modifiers are summarized here and in \changedin{1D}{1D-tables}{the play aid tables}{Table~\ref{table:air-to-air-gun-and-rocket-modifiers}}:
...
\paragraph{Procedure.} The attacking aircraft declares its target and indicates the number of rocket factors being fired. \changedin{1D}{1D-tables}{The Air To Air Rocketry Table}{Table~\ref{table:air-to-air-rocketry}} is consulted. At the intersection of the range column and the rocket factors row is the base die roll to hit. Roll the ten-sided die and modify the result as appropriate; if the result is equal to or less than the base die roll to hit, the attack has produced a hit. The rocket factors entry also shows the Attack Rating for that number of rockets being fired. This Attack Rating is used on \changedin{1D}{1D-tables}{the Damage Tables}{Table~\ref{table:aircraft-damage}} if a hit occurs.
