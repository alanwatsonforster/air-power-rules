...
This rule details how aircraft conduct gun and rocket attacks against other aircraft.
}
...
A gun-armed aircraft may conduct one or two gun attacks per game turn. It may attack at any point during its move, but at least one FP (HFP or VFP) must be used before each shot. 

The internal gun data section on the combat characteristics panel of the aircraft's ADC gives information about its internal guns. Table~\ref{table:stores-GP} gives information on gun pods.
}
...
\Ax{
\paragraph{Gun Attack Prerequisites.}
Gun attacks are restricted as follows:
\begin{itemize}
    ...
    \item An aircraft may not fire while turning or recovering from turning at the ET turn.
    \item An aircraft may not fire after an FP used to prepare for or execute a rolling maneuver.
\end{itemize}
}
...
The hit die roll (from the ADC or Table~\ref{table:stores-GP}) shows the die roll required to hit the target at ranges of 0, 1, and 2 hexes. If only two hit die rolls are given, the gun's range is only one hex. Roll the die, modify the result as appropriate, and compare the modified result to the hit die roll. The attack hits if the modified result is less than or equal to the hit die roll.
}
...
\begin{itemize}
    ...
    The size number from the target's ADC is used directly as a modifier.   
    }
    ...
    \item\itemparagraph{Pilot Quality and Special Characteristics.} Better-trained pilots shoot better, while poorly trained or inexperienced pilots shoot worse. See advanced rule~\ref{rule:crew-quality}. Apply the appropriate modifier from Table~\ref{table:air-to-air-gun-and-rocket-modifiers} for the pilot's quality and special characteristics.
...
\paragraph{Head-On Gun Attacks.} If both the attacker and target directly face each other with the attacker on the 180{\deg} line shown in Figures~\ref{figure:angle-off-facing-hex-corner}, \ref{figure:angle-off-facing-hex-side}, or \ref{figure:angle-off-on-hex-side}, the attack is a head-on attack. A target aircraft waiting to move (or that has already finished its move) may return fire in response to head-on attacks provided it does not exceed the two-shot limit per game turn and satisfies all of the normal prerequisites except the sighting requirement. An aircraft that has not flown yet may \emph{not} respond to a head-on attack if it is carrying or recovering from an ET turn or prepared for or executed a rolling maneuver on the last FP of the previous game turn. Gunsight modifiers apply if it is carrying a turn or recovering from a turn.
}
...
\paragraph{Concept.} Angle-off or deflection is the angle “off” the target's tail. That is, it is the horizontal angle between two imaginary lines, one extending behind the target and another extending from the target to the attacker. If the attacker is directly behind the target, the angle-off is 0{\deg}, and if the attacker is directly in front of the target, it is 180{\deg}. Angle-off is used to determine attack modifiers and for other functions. 

\paragraph{Angle-Off Arcs.} Angle-off is described in 30{\deg} arcs to the left or right of the target. Figures~\ref{figure:angle-off-facing-hex-corner}, \ref{figure:angle-off-facing-hex-side}, and \ref{figure:angle-off-on-hex-side} show these arcs with respect to the target aircraft. The appropriate figure is used according to whether the target aircraft is in a hex facing a hex side, in a hex facing a hex corner, or on a hex side.

\paragraph{0{\deg} and 180{\deg} Lines.} Though not formally arcs, the 0{\deg} and 180{\deg} lines are at times treated as such. The imaginary lines extending behind and ahead of the target aircraft are the 0{\deg} and 180{\deg} lines, respectively. To qualify for modifiers for one of these lines, the attacker must be both on the line and facing along it towards the target.
...
If the attacking aircraft or missile and the target are at the same map location (hex or hex side), the angle-off arc is given by the difference in their facings. A difference of 180{\deg} corresponds to the 180{\deg} line rather than the 180{\deg} arc. For example, if the target is facing N and the attacker is facing NNW, the facing difference is 30{\deg}, and the attacker is in the target's left 30{\deg} arc.

Otherwise, consult whichever of Figures~\ref{figure:angle-off-facing-hex-corner}, \ref{figure:angle-off-facing-hex-side}, and \ref{figure:angle-off-on-hex-side} corresponds to the map location and facing of the target. Often the attacking aircraft or missile will unambiguously be in an angle-off arc. However, sometimes the attacker will be located on the borderline between two arcs. To resolve these cases, consider the speeds of the attacker and target at the start of the game turn:
\begin{itemize}
...
\item If neither is faster or if the attacker remains on the borderline after one of the previous two cases, the attacker is in the arc that gives it the more beneficial modifier.
\end{itemize}

For example, consider Figures~\ref{figure:angle-off-facing-hex-side}, \ref{figure:angle-off-facing-hex-corner}, and \ref{figure:angle-off-on-hex-side} in which the aircraft A are targets and the aircraft B and C are attackers.
Aircraft C in Figure~\ref{figure:angle-off-facing-hex-side} is unambiguously in the right 90{\deg} arc of its target, the other attackers are on borderlines between two arcs. If the target were slower than the attackers, then B would be in the right 150{\deg} arc and C on the right 0{\deg} line in Figure~\ref{figure:angle-off-facing-hex-corner}, B would be able to chose the more beneficial of either the right 60{\deg} or right 90{\deg} arc in Figure~\ref{figure:angle-off-facing-hex-side}, and B would be in the left 120{\deg} arc and C in the left 30{\deg} arc in Figure~\ref{figure:angle-off-on-hex-side}. On the other hand, if the target were faster than the attackers, then B would be in the right 60{\deg} arc in Figure~\ref{figure:angle-off-facing-hex-side}, B would be in the left 90{\deg} arc in Figure~\ref{figure:angle-off-on-hex-side}, and the other arcs would be unchanged.
...
\begin{itemize}
    ...
    \item The fields of IRSTS (rule~\ref{rule:irsts}), VAS (rule~\ref{rule:irsts}), missile seekers (rule~\ref{rule:sighting-aircraft-and-missiles}), radar (rule~\ref{rule:air-to-air-radar}), jammers (rule~\ref{rule:electronic-warfare}), and laser designators (rule~\ref{rule:laser-guided-weapons}).
\end{itemize}

\paragraph{Ranges of Arcs.} Ranges of arcs are used for restricted or blind regions for sighting, sensor fields, and to determine advantage. If a range is given as an arc followed by a $+$, it means that the range is that arc and all more forward arcs. If a range is given as an arc followed by a $-$, it means that the range is that arc and all more rearward arcs. 

For example, a radar search field given as $60{\deg}\!-$ comprises the 30{\deg} and 60{\deg} arcs and a blind arc given as $150{\deg}\!+$ comprises the 150{\deg} and 180{\deg} arcs.

\paragraph{Angle-Off Procedure for Restricted and Blind Arcs.}
In this case, the sighting aircraft takes the place of the target aircraft and the potentially sighted element takes the place of the attacker. 

The procedure is the same as that for air-to-air gun attacks, in that borderline cases are resolved by considering moving the faster element forward, except that if the potentially sighted element is faster and would remains on the borderline if it moved forward, consider the sighting aircraft to be faster.

When checking if a ground unit is in the restricted or blind arc of an aircraft, the ground unit is always considered to be slower than the aircraft.

\paragraph{Angle-Off Procedure for Advantage.}
In this case, the aircraft seeking advantage takes the place of the target aircraft and the potentially disadvantaged aircraft takes the place of the attacker. 

The procedure is the same as that for air-to-air gun attacks, in that borderline cases are resolved by considering moving the faster element forward, except that if the potentially disadvantaged element is faster and would remains on the borderline if it moved forward, consider the aircraft seeking advantage to be faster.

\paragraph{Angle-Off Procedure for Other Cases.}
In other cases, the aircraft or missile sensing, jamming, or designating takes the role of the target aircraft and the potentially sensed, jammed, or designated element takes the role of the attacker. 

The procedure is the same as that for air-to-air gun attacks, but if the potentially sensed, jammed, or designated element falls on the borderline that defines the outer edge of the field, it is considered to be within the field. The relative speed of the two elements is not considered.
...
\item CA (Computer Assisted). The attacker may use CA Radar Ranging with or without SSGT when firing from the 90{\deg} angle-off or less.
