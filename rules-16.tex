...
\paragraph{Die Roll Modifiers.} Detection die roll modifiers exist for electronic jamming, the presence of chaff and mini-jammer programs, stealth technology, crew quality, and/or aircraft size. These are summarized \changedin{1D}{1D-table}{on the Search Modifiers Table}{in Table~\ref{table:radar-search-modifiers}}.
...
\paragraph{Die Roll Modifiers.} The same modifiers that apply to search rolls, apply to lock-on rolls.}{The lock-on modifiers are given in \changedin{1D}{1D-tables}{the play aids}{in Table~\ref{table:radar-lock-on-modifiers}}.}
...
\addedin{1D}{1D-tables}{These rules are summarized in Table~\ref{table:radar-breaking-lock-on}. }
...
\addedin{1D}{1D-tables}{The limitations on radar searches are given here and summarized in Table~\ref{table:radar-search-limitations}. }A pilot only crewed aircraft may not perform normal searches if:
...
\paragraph{Look-Down Technology.} An aircraft with Look Down Technology can ignore all Look Down Limitations and may guide look down capable missiles against targets in ground clutter conditions.
...
\paragraph{Auto-Track Radar Mode.} Auto-Track Radar mode allows an aircraft to automatically detect and then lock-on a target. Only aircraft with Auto-Track Technology may use this mode. \addedin{1D}{1D-tables}{Table~\ref{table:radar-auto-track-mode} summarizes auto-track mode.}
...
One Auto-Track Mode lock-on attempt is allowed against the detected target even if the aircraft violated the normal radar use maneuver restrictions. Auto-Track will ignore all friendly aircraft with IFF on. A visually sighted enemy aircraft may be selected for detection and lock-on even if it was not the closest as long as it meets the range and 180° arc requirements. \addedin{1B}{1B-apj-22-qa and 1B-apj-39-qa}{In multi-crew aircraft, the crew quality modifiers for the pilot are used for auto-track mode.}
