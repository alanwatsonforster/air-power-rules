...
\begin{itemize}
    ...
    \item PSSM aircraft = $+2.0$ decel if any turns \changedin{1B}{JDW in the TSOH errata}{or rolls}{and $+2.0$ decel if any rolls} done, and reduce maximum turn rate by one but not to less than HT.
    \item Normal aircraft = $+1.0$ decel if any turns \changedin{1B}{JDW in the TSOH errata}{or rolls}{and $+1.0$ decel if any rolls} done.
    \item GSSM aircraft = No additional decel for turns or rolls.
    \item If in Mil.\ pwr., \changedin{1B}{JDW in the TSOH errata}{$+1.0$}{$+1.5$} decel per 0.5 speed over High Transonic.
