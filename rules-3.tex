...
This rule describes how the hex grid on the game map is used to position game counters during play. The game map serves to show exactly where aircraft, missiles, and other units are located. Every game counter must be placed on the map to make its position clear to all players.
}

\section{\x{Basic Elements of Position}{Position}}
\label{rule:position}
...
Each aircraft and missile counter has a position defined by its map location, facing, and altitude.
}
...
\begin{itemize}
    ...
    This is the horizontal direction in which an aircraft or missile is flying. Use the silhouette on the counter to show facing by pointing the nose of the aircraft or missile in one of the twelve possible directions shown in Figure~\ref{figure:facing}. Each direction differs by 30 degrees and has a corresponding compass heading associated with it (i.e., N = north, SSE = south-south-east, etc.). In the scenario booklets, next to the map layout diagrams, there will be a compass arrow indicating the direction of north relative to the hex grid.
    }
...
\paragraph{Ground and Naval Unit Counters.} \x{
...
Ground combat and naval units are always placed wholly within hexes and never on hex sides. Heavy AAA units (see rule~\ref{rule:heavy-aaa-unit-facing}) and large naval units must be faced like aircraft and missiles. Other ground units do not consider facing.
}
...
\paragraph{Collision Resolution.} \x{
