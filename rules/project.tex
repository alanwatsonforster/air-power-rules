...
%    \item Version 1E will have additional minor changes from non-authoritative sources.
    ...
    \item Versions up to and including 2B will include only minimal changes to the text. However, version 2C is a rewrite. Certain rules have been rewritten for clarity and conciseness. Parts of the text have been reorganized. However, the meaning of the version 2C rules should be the same as the 2B rules.
...
\begin{itemize}
    \item If you wish to play with second-edition rules with changes only from authoritative sources, I would suggest using version 2A. However, you will miss out on the minor, but non-authoritative, additions in version 2B that cover aspects missing from version 2A and miss out on the additional clarity of version 2C.
    \item If you wish to play with second-edition rules and are prepared to accept non-authoritative minor additions, I would suggest using version 2C.
    \item I use version 3A, as I consider it has significant improvements over 2B especially in turn drag, sighting in adverse weather, and vertical arcs. However, these changes are neither minor nor authoritative.
\end{itemize}    
...
\begin{itemize}
    ...
    \item{2B-merging-cloud-layers} Two dense layers that overlap or are adjacent are merged. A stratus layer that overlaps with a dense layer (but not one that is adjacent) is ignored.
    
    \item{2B-stratus} Aircraft and ground units may not visually sight, use VAS, use IRSTS, use TV/IR Optics, use Night IR sights, track OG/LG SAMs, use laser designators, or use laser spots on targets on the opposite side of stratus cloud layers.
    
    \item{2B-generating-night-conditions} I have added a simple means to determine randomly if a scenario occurs at night. Stating this explicitly in the scenario notes is probably a more satisfactory approach.
    
    \item{2B-aaa-sun-clutter} AAA units stacked with a FCR or with an integrated W-type FCR are not penalized for firing when in the Sun clutter of their target.
...
\begin{itemize}
    ...
    \itemtag{3A-combined-arcs} I have added combined (horizontal and vertical) arcs to the new chapter on ranges and arcs. The rule is adapted from Tony Valle proposal in APJ 39.
    ...
    We similarly average the DPs for supersonic turns over two turns.
    ...
    \item
    The $+2$ modifier applies if the target is in the restricted arc of the searcher, not all participating aircraft.
    
    \item
    The version 1A rules give a $-1$ modifier for a RWR detection, but not other types of detections. In 3A this is changed to be a $-1$ modifier for any type of detection or lock-on.
    
    \item The version 1A rules give a $-1$ modifier for an IRSTS lock-on and HUD interface technology, but not for a radar lock-on. In 3A this is changed to be a $-1$ modifier for any type of lock-on with HUD interface technology.
