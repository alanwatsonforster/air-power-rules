...
This advanced rule details the effects of the crew on aircraft operations. A crew member’s ability is subdivided into quality, attributes, and characteristics, and the implications of these are described in the following sections.
...
\begin{itemize}
    ...
    \item If accompanied by a regular or veteran non-pilot crew member, may perform damage control like a novice.
\end{itemize}
...
\begin{itemize}
    ...
    \item If accompanied by a regular or veteran non-pilot crew member, may perform damage control normally.
\end{itemize}
...
Pilots can become disoriented. Green and novice pilots risk disorientation if they perform certain maneuvers or select certain flight types, as described in advanced rule~\ref{rule:crew-quality}. Any pilot can become disoriented in adverse weather or at night, according to advanced rule~\ref{rule:night-and-weather}.

\paragraph{Pilot Disorientation.}\label{rule:disorientation} Anytime an aircraft performs an action which risks disorientation, roll one die and apply appropriate modifiers from Table~\ref{table:disorientation-avoidance}. If the result is $3-$, the pilot becomes disoriented.
...
\begin{itemize}
    ...
    \item\itemparagraph{Ejection.} Disoriented pilots may eject or bail out normally.
\end{itemize}
...
\begin{itemize}
    ...
    \item \itemparagraph{Decorated Combat Hero.} Upon rolling $2-$ after any single mission in which the players collectively feel the crew member in question performed in an extraordinary manner that deserves a medal. This requirement is vague, but it usually is in real life, too.
...
\paragraph{Ball Out Restrictions.} Bailing out of an aircraft is only allowed if the aircraft is or was at a speed of four or less, and if bailing out of a destroyed aircraft, only if it was four or more levels above the ground. \addedin{2B}{2B-gloc-bail-out}{A crew member suffering GLOC cannot bail out.}
...
\begin{itemize}
    ...
    \item \itemparagraph{Missing in Action (MIA).} An MIA crew member is killed (perhaps drowned at sea, died on the ground, or died in prison).
    \item \itemparagraph{Rescued.} A rescued crew member may be able to re-enter the campaign. Roll one die, the result is the number of campaign days the crew member will miss due to injury or rescue delays. After missing the required number of days, the crew member can be put back on the roster flying missions.
...
\paragraph{Victory Points For Crew Losses.} In campaign games VPs are awarded for capturing or killing opposing crew members. Table~\ref{table:crew-vps} indicates values for crew members that are killed, are MIA, or become POW.
}
