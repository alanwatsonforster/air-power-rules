...
\end{itemize}
Furthermore, an aircraft whose pilot is suffering from GLOC (see rule~\ref{rule:gloc}), is disoriented (see advanced rule~\ref{rule:disorientation}), or is green (see advanced rule~\ref{rule:crew-quality}) may not defensively engage a missile.
...
\paragraph{Engagement Restrictions.} An engaged aircraft is not allowed to make attacks of any sort, may not launch weapons, may not perform damage control (see advanced rule~\ref{rule:damage-control}), may not carry out radar searches (see rule~\ref{rule:radar-searches}), loses all radar lock-ons (see rule\ref{rule:radar-tracking}), may not illuminate a target (see rule~\ref{rule:target-illumination})
