...
Air-to-air missiles can be divided according to their seeker into infrared- and radar-guided missiles. This rule describes the common characteristics of both types of missiles. The specific rules relating to the seekers are given in rule~\ref{rule:infeared-heat-seeking-missiles} for infrared-guided missiles and rule~\ref{rule:radar-guided-missiles} for radar-guided missiles. 

Targets may defend themselves against air-to-air missiles by maneuvering so that they can no longer be tracked by the seeker (see rules~\ref{rule:infeared-heat-seeking-missiles} and \ref{rule:radar-guided-missiles}) by dispensing decoys such as flares, chaff, and mini-jammers (see advanced rule~\ref{rule:dds}), by using radar or infrared jammers (see advanced rules~\ref{rule:radar-jamming} and \ref{rule:infrared-jamming}).
}
...
\begin{itemize}
    ...
    \item \itemparagraph{Seeker.} A letter code indicating the type of seeker the missile has. E, I, M, A, are infeared seekers (Early, Improved, Modern, Advanced).  BR, RH, AH are radar seekers (Beam-Rider, Radar-Homer, Active-Homer). A dash means it is unguided and has no seeker.
...
\begin{itemize}
    ...
    \item \itemparagraph{Seeker.} A letter code indicating the type of seeker the missile has: E, I, M, and A are early, improved, modern, and advanced infeared seekers; BR, RH, and AH are beam-rider, radar-homing, and active-homing seekers; and a dash means that the missile is unguided and has no seeker.
    ...
    Front launches are those taken from the target’s \arc{150} and \arc{180} arcs; side launches are those from the \arc{120} and \arc{90} arcs; rear launches are those from the \arc{60} or \arc{30} arcs. A dash indicates the shot is not allowed.
...
\item At the start of or during the missile’s proportional move, the target occupies one of the locations shown in Figure~\ref{figure:missile-attacks} relative to the missile, the missile has the same altitude as the target, and the missile has at least one FP remaining in its proportional move.
\end{enumerate}
}
