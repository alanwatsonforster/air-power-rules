...
\newcommand{\setfiguresize}[5][black]{
...
\newcommand{\drawaircraftcounter}[7][0]{
    ...
    \begin{athex}{#2}{#3}
        \begin{scope}[rotate=#4-45]
            ...
            \node [anchor=north west,rotate=#4-45+#1] at (-0.32,+0.32) {\tiny #6};
            \node [anchor=south east,rotate=#4-45+#1] at (+0.32,-0.32) {\tiny #7};
        \end{scope}
    \end{athex}
}
...
\newcommand{\drawdottedhex}[3][]{
    \begin{athex}{#2}{#3}
        ...
        \begin{scope}[color=black!25,#1]
            ...
            \draw[fill]  (30:0.5) circle (0.03);
            \draw[fill]  (90:0.5) circle (0.03);
            \draw[fill] (150:0.5) circle (0.03);
            \draw[fill] (210:0.5) circle (0.03);
            \draw[fill] (270:0.5) circle (0.03);
            \draw[fill] (330:0.5) circle (0.03);
        \end{scope}
    \end{athex}
}
...
\newcommand{\drawpositiongrid}[4]{
    ...
    \miniathex{#1}{#2}{
        \foreach \x in {1,3,...,#3} {
            \foreach \y in {1,...,#4} {
                \miniathex{-1.0}{-1.0}{\drawpositions{\x}{\y}}
            }
        }
        \foreach \x in {2,4,...,#3} {
            \foreach \y in {0,2,...,#4} {
...
\newcommand{\drawevenpositiongrid}[4]{
    % Odd columns are one hex shorter than even columns.
    \miniathex{#1}{#2}{
        \foreach \x in {1,3,...,#3} {
            \foreach \y in {1,...,#4} {
        ...
        \foreach \x in {2,4,...,#3} {
            \foreach \y in {2,...,#4} {
                \miniathex{-1.0}{-1.5}{\drawpositions{\x}{\y}}
            }
        }
    }
}
\newcommand{\drawoddpositiongrid}[4]{
    % Even columns are one hex shorter than odd columns.
    \miniathex{#1}{#2}{
        \foreach \x in {1,3,...,#3} {
            \foreach \y in {2,...,#4} {
                \miniathex{-1.0}{-2.0}{\drawpositions{\x}{\y}}
            }
        }
        \foreach \x in {2,4,...,#3} {
            \foreach \y in {1,...,#4} {
