...
Stratus cloud layers are cloud layers that are only one altitude level thick. They block visible and infrared light from one side to the other. The effects of stratus cloud layers are:

\begin{itemize}

\item Aircraft and ground units above or below a stratus cloud layer may not visually sight targets beyond that layer.

Aircraft above or below a stratus cloud layer may not use VAS, IRSTS, TV/IR optics, laser designators, or laser spot trackers to sight, detector, designate, or track a target beyond that layer. They may also not launch IRMs on targets on the other side of the layer, even with radar assist (see rule \ref{rule:irm-radar}).

Ground units below a stratus cloud layer may not use night IR sights to detect aircraft above that layer, may not launch IR SAMs at targets above that layer, and may not launch or track OG/LG SAMs at targets above that layer.

\item Aircraft in a stratus layer can attempt to sight other aircraft and missiles with a modifier of $+2$. 
\item Aircraft and missiles in a stratus layer can be sighted with a modifier of $+3$. 
\item An aircraft in a stratus layer can sight other aircraft and missiles in the same layer to a maximum range of 1 (i.e., in the same or adjacent hexes), and in this case both the $+2$ modifier for the searcher and the $+3$ modifier for target would apply.
\item IRMs and IR/OG/LG SAMs may pursue and attack aircraft in stratus cloud layers. However, if both the missile and the target are in the same layer, then at the end of each of the missile’s proportional moves the range to the target must be at most 1 otherwise the missile fails to track and is removed. If, at the end of any of the missile’s proportional moves, the target and missile are on opposite sides of a stratus cloud layer, the missile fails to track and is removed from play.
\item Air-to-air attacks with visually aimed weapons require the target to be individually sighted by the attacker at the start of the attacker's movement (see rule \ref{rule:individual-sighting}). Exceptionally and for this purpose only, if a potential target moves first and crosses to the other side of a stratus cloud layer, the stratus cloud layer is not considered to block visual sighting.
