...
\paragraph{AAA Aimed Fire Modifier.} AAA units firing on the target from within its Sun clutter  suffer a $+1$ modifier to their hit die roll for aimed fire, unless they are stacked with a FCR or have an integral W-type FCR.
...
\IAY[3A-sighting]{If the target is below the highest dense cloud layer, a $+1$ modifier applies (reflecting the poorer light below dense clouds).}
...
\paragraph{Sighting Aircraft.} Aircraft contrailing are spotted automatically to a sighting range of 24 if they are above the highest cloud layer and 6 otherwise (see rule \ref{rule:contrail-layers}). Otherwise, the maximum visual sighting range for all aircraft is 2 unless the target used AB power or is contrailing, in which case the maximum sighting range is 6. 
    
Sighting modifiers for \CY[3A-sighting]{paint scheme, relative altitude,}{background} and smoking do not apply at night.

\paragraph{Sighting Missiles.} Missiles contrailing are spotted automatically to a sighting range of 24 if they are above the highest cloud layer and 6 otherwise (see rule \ref{rule:contrail-layers}). Otherwise, missiles may only be sighted on the turn after launch, and on each turn, a sustainer is burning. The visibility numbers are halved, and the modifier for having been launched from a sighted aircraft does not apply.
