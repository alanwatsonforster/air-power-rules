...
\item\itemparagraph{Sighting Range.}
If either the searcher or target or both are in a haze layer,  \CY[3A-sighting]{the maximum sighting range for an aircraft is twice its visibility number.}{the sighting range for visual sighting and identification (see rule~\ref{rule:visual-sighting}) is considered to be twice the normal range, which reduces both the maximum range to which the target can be sighted and gives more adverse modifiers. This applies to normal visual sighting (air-to-air, air-to-ground, and ground-to-air) and to the use of VAS. It does not apply to the use of IRSTS or TV/IR optics.}

\IDY[3A-sighting]{\itemparagraph{Sighting Roll Modifiers.} If the target is in haze, use the sighting modifier from Table \ref{table:sighting-position-modifiers} in addition to the normal position modifier.}

\item\itemparagraph{Individual Sighting.} Air-to-air attacks with visually aimed weapons require the target to be individually sighted by the attacker at the start of the attacker's movement (see rule \ref{rule:individual-sighting}). Exceptionally and for this purpose only, if a potential target moves first and enters a haze layer, \CY[3A-sighting]{the maximum sighting range is considered to be the normal value of four times the visibility number and not twice the visibility number}{the sighting range is considered to be the actual range and not twice the actual range}. This reduces the opportunity for escape that a haze layer offers a target.
...
\item\itemparagraph{Visual Sighting Modifiers.} Aircraft between a cloud layer and the searcher are considered to be seen against clouds for determining the sighting roll modifier from Table~\ref{table:sighting-position-modifiers}. \AY[3A-sighting]{If the target is below the highest cloud layer, a $+1$ modifier applies (reflecting the worse light below clouds).}
...
\item\itemparagraph{Sighting Roll Modifiers.} Stratus cloud layers give the following sighting modifiers:
\begin{itemize}
...
\item\itemparagraph{Individual Sighting.} Air-to-air attacks with visually aimed weapons require the target to be individually sighted by the attacker at the start of the attacker's movement (see rule \ref{rule:individual-sighting}). Exceptionally and for this purpose only, if a potential target moves first and crosses to the other side of a stratus cloud layer or enters the same layer as the attacker, the stratus cloud layer is not considered to block visual sighting. This reduces the opportunity for escape that a stratus cloud layer offers a target.
...
As an exception, if the target of an IRM or IR SAM enters a dense layer, the missile is allowed to follow during its next proportional move. If it cannot attack during this proportional move, it fails to track and is removed from play. This reduces the opportunity for escape that a dense cloud layer offers a target.
