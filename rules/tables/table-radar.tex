...
\toprule
\addlinespace
Aircraft may not search if they:\\
\begin{itemize}[topsep=0pt]
 \item Turned at BT or greater.$^\dagger$
 \item Turned at HT if pilot is radar operator.$^\dagger$
 \item Used more than one VR.$^\dagger$
 \item Used any other special maneuver except a slide.$^\dagger$
 \item Unloaded and have not yet recovered.$^\dagger$
 \item Were stalled, departed, or engaged
 \item Received H or C damage or performed damage control.
 \item Made an air-to-air gun or rocket attack.
 \item Made an air-to-ground attack.
 \item Has their radar operator in GLOC
 \item Has their radar operator suffering disorientation.
\end{itemize}\\
$\dagger$ Does not apply to boresight or auto-track modes.\\
\addlinespace
Targets of searches must be:\\
\begin{itemize}[topsep=0pt]
    \item Within radar arc.
    \item Within radar vertical limits.
    \item Within search range.
    \item Not in ground clutter (unless look-down or limited look-down radar).
\end{itemize}\\
\bottomrule
\end{tabularx}
\end{onecolumntable}
...
\begin{twocolumntable}
\tablecaption{table:radar-searches}{Radar Search Ranges}
...
\begin{twocolumntable}
\tablecaption{table:radar-search-and-lock-on-modifiers}{Radar Search and Lock-On Modifiers}
\begin{tabularx}{0.9\linewidth}{lLccl}
\toprule
Description&Modifier&Search\tablenotemark{1}&Lock-On\tablenotemark{2}&Rule\\
\midrule
...
Target using chaff DDS&+PPL modifier\tablenotemark{3} $-$ half ECCM rating\tablenotemark{4}&Y&Y&\ref{rule:dds}\advancedrulemark\\
Target using mini-jammer DDS&+PPL modifier\tablenotemark{3}&Y&Y&\ref{rule:dds}\advancedrulemark\\
...
\advancedruletext
\tablenotetext{1}{The modifier applies to search attempts if Y appears in this column.}
\tablenotetext{2}{The modifier applies to lock-on attempts if Y appears in this column.}
...
\addlinespace
\begin{itemize}[topsep=0pt]
    \item Aircraft stalls, departs, or engages.
    \item Aircraft does ET turn, displacement, lag, or barrel roll, or VIFF manueuver.
    \item Aircraft suffers a H or C hit,
    \item Radar operator GLOCs.
    \item Target cannot be kept in radar arc
    \item Target deploys chaff or mini-jammers and rolls effectiveness or less.
    \item Target employs DJM and rolls DJM rating $-$ ECCM rating or less.
\end{itemize}
\\
\bottomrule
\end{tabularx}
...
\tablecaption{table:radar-boresight-mode}{Boresight Radar Mode}
\begin{tabularx}{\linewidth}{X}
\toprule
\addlinespace
\begin{itemize}[topsep=0pt]
    \item Previous detections and lock-ons lost when mode declared.
    \item No flight restrictions.
    \item Radar arc is limited.
    \item Maximum range is the tracking strength.
    \item The nearest sighted aircraft in radar arc is automatically detected. 
    \item Lock-on roll allowed.
\end{itemize}
\\
\bottomrule
\end{tabularx}
...
\tablecaption{table:radar-auto-track-mode}{Auto-Track Radar Mode}
\begin{tabularx}{\linewidth}{X}
\toprule
\addlinespace
\begin{itemize}[topsep=0pt]
    \item Previous detections and lock-ons lost when mode declared.
    \item No flight restrictions.
    \item Radar arc is \arcrange{180}{+}.
    \item Maximum range is the search strength.
    \item Any visually sighted aircraft in the radar arc may be selected for detection by rolling 7 or less.
    \item Otherwise, the nearest aircraft in radar arc is automatically contacted. Friendly with IFF on are ignored.
    \item Lock-on roll allowed.
\end{itemize}
\\
\bottomrule
