...
To be eligible for padlocking, a target must have been sighted on the previous game turn, must be within maximum visual sighting range of the padlocking aircraft, and must not be in its blind arc. Table~\ref{table:padlocks} summarizes these requirements and others covered by the advanced rules.
...
\begin{itemize}
    ...
    \end{itemize}
    \item\itemparagraph{Searcher has Detection.} If the searcher has a radar, IRSTS, or RWR detection (or lock-on) of the target, apply a $-1$ modifier. This modifier is applied only once regardless of how many different types of detections the searcher has of the target.
    \item\itemparagraph{Searcher has Lock-On and HUD Interface.} If the searcher has a radar or IRSTS lock-on for the target and HUD interface technology, apply an additional $-1$ modifier. Again, this modifier is applied only once regardless of how many different types of lock-ons the searcher has for the target.
