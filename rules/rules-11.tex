...
\begin{itemize}
    ...
    \item \itemparagraph{Restricted Arc.} If the target is in the restricted arcs of \CY[3A-restricted-arc]{\emph{all} of the participants}{the searcher}, apply a $+2$ modifier.
    ...
    \IAY[3A-sighting-modifiers]{\itemparagraph{Other Detections.} If the searcher has a radar, IRSTS, or RWR detection (or lock-on) for the target, apply a $-1$ modifier. This modifier is applied only once regardless of how many different types of detections the searcher has of the target.}
    \IAY[3A-sighting-modifiers]{\itemparagraph{Other Detections and HUD Interface.} If the searcher has a radar or IRSTS detection (or lock-on) and HUD interface technology, apply an additional $-1$ modifier. Again, this modifier is applied only once regardless of how many different types of detections the searcher has of the target.}
...
\begin{itemize}
    ...
    \item IRSTS-B (advanced) covers the \arcrange{180}{+} \CY[3A-combined-arcs]{arc}{combined arcs (see rule \ref{rule:combined-arcs})} and successfully locks-on on a roll of $8-$.
\end{itemize}
