...
\paragraph{Blind and Restricted Arcs.} A crewmember’s view may be obstructed by their aircraft’s fuselage. Each aircraft’s ADC indicates its blind and restricted \AY[3A-horizontal-arcs]{horizontal} arcs (if any). An L after the arc indicates that the aircraft is blinded or restricted only to lower altitude in that arc (e.g., by a long nose). An aircraft cannot padlock or attempt to sight targets in its blind arcs. 

\AY[3A-horizontal-arcs]{
When determining the horizontal arc, the aircraft attempting sighting is the reference element and the aircraft or missile being sighted is the distant element. Resolve borderline cases by first moving the faster element forward. If the faster element remains on the borderline, use the rearward arc.
}
...
\begin{itemize}
    ...
    \item \itemparagraph{Restricted Arc.} If the target is in the restricted arcs of \CY[3A-restricted-arc]{}\emph{all} of the participants}{the searcher}, apply a modifier of $+2$.
    ...
    \item \itemparagraph{Searcher has Poor or Excellent Eyesight.} If the searcher has poor eyesight, apply a $+1$ modifier, and if they have excellent eyesight, apply a $-1$ modifier.
    \item \itemparagraph{Searcher is a Tactics Master.} If the searcher is a tactics master, apply a modifier of $-1$.
    \IDY[3A-sighting-modifiers]{\itemparagraph{Searcher has IRSTS Lock-on and HUD Interface.} If the searcher has an IRSTS lock-on to the target and HUD interface technology, apply a $-1$ modifier.}
    \IDY[3A-sighting-modifiers]{\itemparagraph{Searcher has RWR Indications.} If the searcher has RWR indications, apply a modifier of $-1$.}
    \IAY[3A-sighting-modifiers]{\itemparagraph{Other Detections or Lock-Ons.} If the searcher has a radar or RWR detection or an IRSTS lock-on, apply a modifier of $-1$.}
    \IAY[3A-sighting-modifiers]{\itemparagraph{Other Detections or Lock-Ons and HUD Interface.} If the searcher has a radar detection or an IRSTS lock-on and a HUD interface, apply an additional modifier of $-1$.}
...
\begin{itemize}
    ...
    \item The target is within visual sighting range of the aircraft checking for individual sighting, and
...
\begin{itemize}
    ...
    \item attempt to sight a target already detected by radar in the searcher’s \arcrange{180}{+} \CY[3A-combined-arcs]{angle-off}{combined} arcs and at a range of no more than 10 times the target’s visibility number, or

    \item attempt to sight a target in the searcher’s limited \AY[3A-combined-arcs]{combined} arc and at a range of no more than 6 times the target’s visibility number (whether previously detected or not).
...
\begin{itemize}
    \item IRSTS-A (early) covers the limited \AY[3A-combined-arcs]{combined} arc and successfully locks-on on a roll of $6-$.
    \item IRSTS-B (advanced) covers the \arcrange{180}{+} \AY[3A-combined-arcs]{combined} arc and successfully locks-on on a roll of $8-$.
\end{itemize}
...
\paragraph{IRSTS Lock-on.} One lock-on attempt is allowed against either the nearest detected target or the target with the highest fuel point use. For this purpose, the fuel use is doubled if the IRSTS is used in the target’s \arcrange{30}{-} \AY[3A-combined-arcs]{combined} arc. Roll the die and compare to the lock-on success requirements.
