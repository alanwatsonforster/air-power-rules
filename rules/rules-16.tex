...
This rule describes how aircraft use their radars to search for and track other aircraft. 
}
...
The arc may be the word “limited” or an arc range (see rule~\ref{rule:ranges-of-angle-off-arcs}). Figure~\ref{figure:limited-arcs} shows the limited arc. In this figure, only the locations within the limited arc line are within the limited arc. The arc ranges \arcrange{180}{+}, \arcrange{150}{+}, \arcrange{120}{+} include the specified arc and all more forward arcs (see rule~\ref{rule:ranges-of-angle-off-arcs}). For example, \arcrange{150}{+} includes the \arc{150} and \arc{180} arcs.
...
The target of a radar search must satisfy these requirements:
    ...
    Aircraft with limited look-down technology may ignore the last restriction. Aircraft with (full) look-down technology may ignore all three restrictions.
...
Boresight radar mode slaves the radar to the gunsight and as such has many limitations compared to normal radar mode. However, it has the advantage that it can be used regardless of the maneuver restrictions that apply to normal radar use.

All aircraft with air-to-air radar can use boresight radar mode. Aircraft with radar that has a lock-on capability but no search capability can only use boresight mode to achieve a lock-on. Table~\ref{table:radar-boresight-mode} summarizes boresight mode.
...
\paragraph{Boresight Radar Mode Requirements.} In order to use boresight radar mode in the air-radar search and lock-on phase, the aircraft must satisfy the requirements for normal radar mode given in rule \ref{rule:radar-search-requirements}, except those related to turn rate, special maneuvers, or unloaded FPs.

% ISSUE: Visually sighted enemy? That is, are friendly aircraft ignored?

\paragraph{Boresight Radar Search Procedure.}  In the air-radar search and lock-on phase, normal radar search is not allowed. Instead, the closest \emph{visually sighted} aircraft in the aircraft's \emph{limited} arc at a range no greater than the radar's \emph{tracking} strength in hexes is automatically detected. If two or more aircraft are equally close, randomly determine which is detected. 
...
Jamming has no effect on this detection. 
...
An aircraft without look-down technology may use boresight mode to detect and lock-on to low-level visually sighted targets
In this case, the automatic contact and the lock-on attempt are allowed, and the lock-on can be subsequently maintained as long as the difference between target level and ground level is more than the searcher and target altitude level difference. The lock-on die roll is subject to a modifier of $-2$. 
}
...
Auto-track radar mode allows an aircraft to automatically detect and then lock-on a target, in a similar way to boresight radar mode but with more capabilities. Again, it has the advantage that it can be used regardless of the maneuver restrictions that apply to normal radar use.
...
\paragraph{Auto-Track Radar Mode Requirements.} In order to use auto-track radar mode in the air-radar search and lock-on phase, the aircraft must satisfy the requirements for normal radar mode given in rule \ref{rule:radar-search-requirements}, except those related to turn rate, special maneuvers, or unloaded FPs.

\paragraph{Auto-Track Radar Search Procedure.}  In the air-radar search and lock-on phase, normal radar search is not allowed. 
Instead, the radar's arc is considered to be the \emph{\arcrange{180}{+}} arc and its range is the \emph{search} strength in hexes. Then, one of two procedures may be chosen:
\begin{enumerate}
\item Any one sighted aircraft in the radar arc and within range may be detected on a roll of $7-$ with no modifiers.
\item Alternatively, the closest aircraft in the aircraft's can be automatically detected. If two or more aircraft are equally close, randomly determine which is detected. Friendly aircraft with their IFF on are ignored in auto-track mode.
\end{enumerate}
...
BJMs and AJMs have no impact on auto-track radar detections, but can impact lock-on attempts.
