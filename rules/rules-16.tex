...
\paragraph{Boresight Radar Search Procedure.} Normal radar search is not allowed in the air-radar search and lock-on phase. Instead, the closest \emph{visually sighted} aircraft in the aircraft's \emph{limited} arc at a range no greater than the radar's \emph{tracking} strength in hexes is automatically detected. If two or more aircraft are equally close, randomly determine which is detected. 
...
\paragraph{Auto-Track Radar Search Procedure.} Normal radar search is not allowed in the air-radar search and lock-on phase. Instead, the radar's arc is considered to be the \emph{\arcrange{180}{+}} arc and its range is the \emph{search} strength in hexes. Then, one of two procedures may be chosen:
\begin{enumerate}
\item Any one sighted aircraft in the radar arc and within range may be detected on a roll of $7-$ with no modifiers.
\item Alternatively, the closest aircraft in the radar arc and within range may be automatically detected. If two or more aircraft are equally close, randomly determine which is detected. Auto-track mode ignores friendly aircraft with their IFF on.
\end{enumerate}
