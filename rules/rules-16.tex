...
To use its radar for an air-to-air search or tracking attempt, an aircraft must satisfy these common requirements:
...
The target of an air-to-air search or tracking attempt must satisfy these common requirements:
...
\begin{itemize}
        ...
        \item It does not prepare for or execute any special maneuvers except slides and vertical rolls. It may perform only one vertical roll per game turn.
...
\item At the end of the preceding flight phase, the target of the search:
\begin{itemize}
    \item Is undetected by the searching aircraft.
    \item Has a range that does not exceed the search range given in the radar section of the ADC.
    \item Satisfies the common radar target requirements (see rule \ref{rule:common-radar-target-requirements}).
\end{itemize}
...
An aircraft may attempt to convert a detection into a track. In addition to the other benefits mentioned above, the maneuvering requirements for continuing to track a target are relaxed compared to those for continuing to detect a target, as the radar automatically adjusts to follow the target it is tracking.
...
\begin{itemize}
\item The aircraft does not select a special radar mode.
\item During the flight phase, the aircraft:
\begin{itemize}
\item Does not use the ET turn rate.
\item Does not prepare for or execute any special maneuvers except slides and vertical rolls. It may perform more than one vertical roll per game turn.
\item Does not declare a defensive preemption (see rule \ref{rule:defensive-preemptions}).
\end{itemize}
\item At the end of the flight phase, the target:
\begin{itemize}
\item Satisfies the common radar requirements above.
...
\begin{itemize}
    \item Is \emph{visually sighted}.
    \item Has a range that does not exceed the radar's \emph{tracking strength}.
    \item Satisfies the common radar target requirements (see rule \ref{rule:common-radar-target-requirements}) for a limited arc.
\end{itemize}
...
\begin{itemize}
    ...
    \item Satisfies the common radar target requirements (see rule \ref{rule:common-radar-target-requirements}) for a \arcrange{180}{+} arc.
\end{itemize}
