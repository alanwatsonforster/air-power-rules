...
The radar and technology sections of the ADC give the characteristics of an aircraft's air-to-air radar. 

Not all aircraft have radars with air-to-air capability. Some have no radar whatsoever. Others have navigation and ground attack radars with no air-to-air capability. In the rest of this rule only applies to radars with air-to-air capability.

\paragraph{Radar Arc.} The arcs line of the radar data section of the ADC shows the arc in which the aircraft’s radar operates when used in normal mode. (The arc may differ in the special radar modes, boresight mode and auto-track mode, described in advanced rules \ref{rule:boresight-mode} and \ref{rule:auto-track-mode}.)
...
\paragraph{Radar Search Data.} The search line of the radar data section of the ADC shows the radar's search capability. If this line has two numbers separated by a dash, the radar is capable of air-to-air search. The first number is the maximum search range in hexes. The second number is the radar search strength rating. 

If the line is empty, contains only a dash, or contains “Gr. Nav.” followed by a number in parentheses, the radar is not capable of air-to-air search.

\paragraph{Radar Tracking Data.} The tracking line of the radar data section of the ADC shows the radar's tracking capability. 

If this line has two numbers separated by a dash, the radar is capable of air-to-air tracking. The first number is the maximum tracking range in hexes. The second number is the radar tracking strength rating. 
...
\paragraph{Radar Modes.} Radars may be switched off or operated in normal, boresight, or auto-track mode. The later two modes are collectively known as special modes. Searching and tracking in normal mode are described in rules~\ref{rule:normal-mode}. The special modes are described in advanced rules \ref{rule:boresight-mode} and \ref{rule:auto-track-mode}. Switching off a radar prevents it from being detected by RWR-C and RWR-D (see advanced rule \ref{rule:rwr})

Whether a radar is switched off or the mode in which is is being operator are declared in the aircraft decisions phase. The only restriction is that auto-track mode may only be selected by aircraft with auto-track technology (see advanced rule \ref{rule:radar-technology}).
...
Whether a target is in radar ground clutter depends on the altitude of the target, the relative position the aircraft using radar, and the technology of the radar. The relevant technologies are limited look-down technology and full look-down technology (see advanced rule \ref{rule:radar-technology}).     Table~\ref{table:radar-look-down} summarizes these rules.

\input{tables/table-radar-look-down}

\paragraph{No Look-Down Technology.}
For a radar with no form of look-down technology, a target that is zero to ten levels above the ground is in ground clutter unless one of the following conditions is met:
...
In normal mode, an aircraft's radar operator actively interacts with the radar system to search for and track targets. This allows the radar to operate with optimal sensitivity and flexibility, but imposes significant limitations on the maneuvers that it can perform.
...
\begin{itemize}
        ...
        \item Perform more than one vertical roll.
    	\item Prepare for or execute any rolling maneuver other than a vertical roll, any VIFF maneuver, or a vertical reverse maneuver.
...
\begin{itemize}
    \item Have a range that does not exceed the radar's search range.
    \item Satisfy the common radar target requirements (see rule \ref{rule:common-radar-target-requirements}).
\end{itemize}
...
\begin{itemize}
\item It and the target satisfy the normal mode search requirements above.
\item It does not switch off its radar or select a special radar mode.
\item It does not start tracking a target (unless it has some form of track-while-scan technology, described in advanced rule \ref{rule:track-while-scan-technology}).
\end{itemize}
...
Boresight mode locks the radar to the gunsight and, as such, has many limitations compared to normal  mode. However, it has the advantage that it does not have the maneuver restrictions that apply in normal mode.

If the radar has a tracking capability but no search capability, it can only use boresight mode to achieve a track. 

Table~\ref{table:radar-boresight-mode} summarizes boresight mode.

\paragraph{Declaring Boresight Mode.} All aircraft with air-to-air radar can use boresight mode. An aircraft may be declared to be using boresight mode in the aircraft decisions phase. 
...
Table~\ref{table:radar-auto-track-mode} summarizes auto-track mode.
...
\begin{itemize}
    ...
    \item Satisfy the common radar target requirements (see rule \ref{rule:common-radar-target-requirements}) for a \arcrange{180}{+} arc. The \arcrange{180}{+} arc is used even if the radar's arc in normal mode is wider.
\end{itemize}
...
\begin{itemize}
    ...
    \item The target satisfies the common radar target requirements (see rule \ref{rule:common-radar-target-requirements}) for a \arcrange{180}{+} arc.
\end{itemize}
