...
\paragraph{Head-On Gun Attacks.} If both the attacker and target directly face each other with the attacker on the \arc{180} line shown in Figure~\ref{figure:angle-off}, the attack is a head-on attack. A target aircraft waiting to move (or that has already finished its move) may return fire in response to head-on attacks provided it does not exceed the two-shot limit per game turn and satisfies all of the normal requirements except the sighting requirement. An aircraft that has not flown yet may \emph{not} respond to a head-on attack if it is carrying or recovering from an ET turn or prepared for or executed a rolling maneuver on the last FP of the previous game turn. Gunsight modifiers apply if it is carrying a turn or recovering from a turn.
}
