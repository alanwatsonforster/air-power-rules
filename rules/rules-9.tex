...
\begin{itemize}
    ...
    A climbing or diving aircraft firing at a target in the same map location  (i.e., the same hex or hex side) may be subject to a vertical-attack modifier. If the attacker is climbing and the target is diving, or vice versa, the modifier is $+2$. If the attacker is climbing or diving and the target is in level flight\AY[3A-vertical-attacks]{ or if the attacker is in level flight and the target is climbing or diving}, the modifier is $+1$. If both the attacker and target are both in level flight, both climbing, or both diving, there is no modifier.
    }{
    If an aircraft fires at a target in the same map location (i.e., the same hex or hex side) may be subject to a vertical-attack modifier. If the attacker is climbing and the target is diving, or vice versa, the modifier is $+2$. If the attacker is climbing or diving and the target is in level flight or if the attacker is in level flight and the target is climbing or diving, the modifier is $+1$. If both the attacker and target are both in level flight, both climbing, or both diving, there is no modifier.
    }
