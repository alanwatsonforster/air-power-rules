...
\label{rule:infra-red-heat-seeking-missiles}
...
\begin{itemize}
    ...
    \item If the launching aircraft used level flight during the game turn of launch, the difference in altitude between the firer and target may not be more than one level for every two full hexes of horizontal range.

    \end{itemize}
    Advanced rule~\ref{rule:irm-realistic-vertical-limits} can replace these simple vertical limits.
...
\paragraph{IRM Launches Inside Envelope Minimum Range.} An IRM may be launched at a target inside its envelope's minimum range. However, range 0 launches are not allowed and range 1 launches for IRMs that are not instant-arming automatically fail. The launch die roll has a modifier of $+3$.

\paragraph{IRM Launches Beyond Envelope Maximum Range.}
IRMs with type A seekers can lock-up and launch at targets beyond the usual envelope maximum ranges:
\begin{itemize}
\item If a target qualifies for a reduced envelope (see advanced rule~\ref{rule:expanded-and-reduced-irm-envelopes} above), an IRM with a type A seeker may be launched up to the normal envelope maximum range.
\item If a target qualifies for neither a reduced nor an expanded envelope (see advanced rule~\ref{rule:expanded-and-reduced-irm-envelopes} above), an IRM with a type A seeker may be launched at up to 1.5 times the normal envelope maximum range (rounding up)
\end{itemize}
