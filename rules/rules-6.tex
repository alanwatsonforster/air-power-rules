...
\paragraph{Deceleration Points.} Deceleration points (DPs) represent the energy lost from low engine power, turning, maneuvering, and climbing flight. The advanced rules discuss additional sources of DPs. DPs cancel an equal number of APs and in that sense can be thought of as negative APs.
}
...
\paragraph{Speed Change Procedure.} In the Aircraft Administrative Phase, determine the aircraft’s new speed for the next game turn according to this procedure. 

Sum the number of APs and the number of DPs accrued during the game turn, including any carried forward from the previous game turn.

\begin{itemize}
\item If the aircraft has an equal number of APs and DPs, it does not change speed.

The aircraft carries no APs or DPs forward into the next game turn.

\item If the aircraft has more APs than DPs, subtract the DPs from the APs, and then increase the speed by 0.5 for every 2.0 remaining APs. However, if the new speed reaches the maximum speed or dive speed, according to rule~\ref{rule:speed-limits}, it is not increased further.

The aircraft carries any APs not used to increase speed forward to the next game turn. 

\item If the aircraft has more DPs than APs, subtract the APs from the DPs, and then decrease the speed by 0.5 for every 2.0 remaining DPs. However, if the new speed reaches 0.0, it is not reduced further.

The aircraft carries any DPs not used to decrease speed forward to the next game turn. 

\end{itemize}

These rules for carrying APs and DPs are further subject to these exceptions at low and high speeds:
\CY[3A-AP-carry-at-speed-limits]{
\begin{itemize}
\item An aircraft with a new speed of 0.0 does not carry DPs forward to the next game turn.
\item An aircraft with a new speed equal to its maximum speed or dive speed, according to rule \ref{rule:speed-limits}, may carry at most 1.5 APs forward to the next game turn.
\end{itemize}
}{
\begin{itemize}
\item An aircraft with a new speed of 0.0 does not carry DPs forward to the next game turn.
\item An aircraft with a new speed equal to its maximum speed or dive speed, according to rule \ref{rule:speed-limits}, does not carry APs forward to the next game turn.
\item An aircraft that suffers a speed fadeback or has its speed reduced to its dive speed, according to rule \ref{rule:speed-limits}, does not carry APs or DPs forward to the next game turn.
\end{itemize}
}
...
For example, an aircraft has accumulated 6.0 APs and 3.0 DPs and so has a net of 3.0 APs. Its speed increases by 0.5, and it carries 1.0 AP forward to the next game turn. Another aircraft has accumulated 2.5 APs and 7.0 DPs, and so has a net of 4.5 DPs. Its speed is reduced by 1.0, and it carries 0.5 DP  forward to the next game turn. These examples assume that the aircraft does not run up against the minimum and maximum speed limits mentioned as exceptions.
}
...
\paragraph{Rapid Acceleration Aircraft.} If its ADC notes that an aircraft has rapid acceleration (RA), it is considered to be an exceptionally clean design that speeds up quicker than normal. This kind of aircraft receives a speed increase of 0.5 FP for each 1.5 APs instead of each 2.0. It decelerates normally.

For example, an RA aircraft has accumulated 6.0 APs and 2.0, and so has a net of 4.0 APs. Its speed is increased by 1.0, and 1.0 AP is carried forward to the next game turn.
}
\DX{
...
\AX{
\paragraph{Fractional APs and DPs.} APs and DPs points are almost always received in steps of 0.5. If the aircraft is damaged, its engine power APs may be halved, and so it might receive APs in steps of 0.25.
}
...
\paragraph{Power Settings.} The four power settings of jet engines and their codes are: idle (I), normal (N), military (M), and afterburner (AB).
...
The four power settings of the engine of a propellor aircraft (with either reciprocating and turboprop engines) and their code are: idle (I), normal (N), half-throttle (HT), and full-throttle (FT). 

The idle and normal settings have the same behavior as those of jet engines. 
...
The ADCs of some propellor aircraft note that they have their power reduced at high speed or altitude. Such reductions apply to both the half-throttle and full-throttle settings.
}
...
\begin{itemize}
    ...
    It it uses turning flight (even at the EZ rate), it receives 2.0 additional DPs for the game turn instead of 1.0.
    }
    ...
    If it performed rolling maneuvers, it receives 2.0 additional DPs per roll instead of 1.0.
    }
...
\begin{itemize}
    ...
    If is uses turning flight, it receives no additional DPs for the game turn instead of 1.0.
    }
    ...
    If it performed rolling maneuvers, it receives no additional DPs per roll instead of 1.0.
    }
