...
\paragraph{Speed Change Procedure.} In the Aircraft Administrative Phase, determine the aircraft’s new speed for the next game turn according to this procedure. Subtract the number of DPs from the number of APs. 

\begin{itemize}
\item If the aircraft has an equal number of APs and DPs, it does not change speed.
\item If the aircraft has more APs than DPs, subtract the DPs from the APs, and then increase the speed by 0.5 for every 2.0 remaining APs. Any APs not used to increase speed are carried forward to the next game turn. 

However, if the new speed reaches the maximum speed or maximum dive speed, according to rule~\ref{rule:speed-limits}, it is not increased further.
\item If the aircraft has more DPs than APs, subtract the APs from the DPs, and then decrease the speed by 0.5 for every 2.0 remaining DPs. Any DPs not used to increase speed are carried forward to the next game turn. 

However, if the new speed reaches 0.0, it is not reduced further.
\end{itemize}
...
\item An aircraft with a new speed equal to its maximum speed or maximum dive speed, according to rule \ref{rule:speed-limits}, may carry at most 1.5 APs forward to the next game turn.
\end{itemize}
}{
\begin{itemize}
\item An aircraft with a new speed of 0.0 does not carry DPs forward to the next game turn.
...
\item An aircraft that suffers a speed fadeback or has its speed reduced to its maximum dive speed, according to rule \ref{rule:speed-limits}, does not carry APs or DPs forward to the next game turn.
\end{itemize}
}
...
If an aircraft is in level flight, climbing flight, or diving flight in which it only loses one altitude level, then the speed change procedure in rule \ref{rule:speed-change} cannot increase its speed above its \emp{maximum speed} for the altitude band in which it ends its flight. Furthermore, if the aircraft’s new speed equals its maximum speed, it carries no APs forward to the next game turn.
