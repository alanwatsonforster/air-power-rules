...
Exceptionally, when sighting a \emph{higher} aircraft or missile in daylight, the vertical range is the difference in altitude levels divided by \emph{four} and rounded down (see rule~\ref{rule:sighting-aircraft-and-missiles}).

\item
The \emph{range} is the sum of the horizontal and the vertical ranges.
\end{itemize}

The horizontal range, vertical range, and range are always non-negative integers. Furthermore, they are symmetric, in the sense that the horizontal range, vertical range, and range from a first element to a second are the same as from the second element to the first.
