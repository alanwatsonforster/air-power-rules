...
\begin{itemize}
    ...
    \item Each altitude band \addedin{1B}{JDW in the APJ 23 errata}{typically }is 8,000 to 10,000 feet thick.
...
\caption{\protect\x{Game Counters}{The game counter on the left represents an F-4 Phantom II and the one on the right an air-to-air missile.}}
\label{figure:game-counters}
\end{figure}
}
...
\paragraph{5. Game Maps.} The game maps are the surface over which the playing pieces will be moved. Though several are provided, sometimes only a few will be used in a scenario. The hexagon grid on each map provides spaces in which players place and move their counters. Like a chessboard, the hexagons help to clearly define where a counter is. Distance between counters (range) is determined by counting the number of hexes between them. \addedin{1B}{JDW in the APJ 23 errata}{Altitude differences may affect range calculations.}

For example, \changedin{1D}{AMW}{as}{Figure~\ref{figure:game-map} shows two game counters on a game map. As} each hex represents {\onethird} of a mile of distance, and as the two aircraft counters shown in the diagram below are 6 hexes apart, they are a scale two miles apart. 
\deletedin{1D}{AWF}{\addedin{1B}{JDW in the APJ 23 errata}{[The two aircraft counters were left off the diagram.]}}
...
\caption{\protect\x{Game Maps}{The games maps are printed with a hex grid to clearly define the positions of game counter and determine the horizontal distances between them. The two aircraft game counters are six hexes apart horizontally. The larger “megahexes” are fives hexes across and can be used to count long distances in fives.}}
\label{figure:game-map}
\end{figure}
}
