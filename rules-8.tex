...
\paragraph{ZC VFPs.} At least one and at most 2/3 of the FPs may be VFPs. ZC VFPs produce an altitude gain based on the aircraft's CC:
\begin{itemize}
...
\paragraph{SC VFPs.} At least one FP must be a VFP. The number of VFPs available and the altitude gain they produce depends on the aircraft’s CC:
...
\paragraph{VC Prerequisite.} To use a VC, an aircraft must have climbed in the previous game turn, unless its ADC notes that it has a high pitch rate and its speed is 3.5 or less, in which case it may enter a vertical climb from level flight.
...
\paragraph{SD Procedure.} Declare diving flight and note SD as flight type on the aircraft log. 

\paragraph{SD VFPs.} At least one and at most $2/3$ of the FPs may be VFPs. SD VFPs produce an altitude loss of  1 or 2 altitude levels per VFP.
