...
\begin{itemize}
    ...
    \item\itemparagraph{ZC Altitude Gain:} ZC VFPs produce an altitude gain based on the Climb Capability Chart. Refer to the CCC on the ADC and find the appropriate configuration, power setting, and altitude band.  If the climb capability is 2.0 or less, one VFP produces a gain of 1 altitude level; if the climb capability is more than 2.0, one VFP produces a gain of \addedin{1b}{JDW: TSOH errata}{1 or} 2 altitude levels.

    \item\itemparagraph{ZC Restrictions:} Aircraft in a Zoom Climb may not use the ET turn rate.

    \item\itemparagraph{ZC Decel Points:} In the first turn of a Zoom Climb, the aircraft receives 1.0 decel point per altitude level gained; on subsequent consecutive turns of ZC, the aircraft receives 1.5 del points per altitude level gained.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item If the \changedin{1B}{JDW: TSOH errata}{value is 1.0 to 1.5}{CCC value is 1.0 to 2.0}, then up to 2/3ds the FPs may be VFPs and the first VFP gains any fraction and the rest gain 1 level each.
        \item If the \changedin{1B}{JDW: TSOH errata}{value is 2.0 or more}{CCC value is greater than 2.0}, then up to 2/3ds the FPs may be VFPs and each may gain 1 or 2 levels.
    \end{enumerate}
...
\begin{itemize}
    ...
    \item\itemparagraph{SD Accel Points:} On the first turn of steep diving, the aircraft receives 0.5 accel points per altitude level lost; on subsequent turn of continued diving, it receives 1.0 accel points per altitude level lost.
...
\begin{itemize}
    ...
    \item\itemparagraph{UD Restrictions:} An aircraft spending unloaded HFPs may not conduct any attacks, aim, track targets or launch weapons.

     \item\itemparagraph{UD Accel Points:} On the first turn of an UD, the aircraft receives 0.5 accel points per altitude level lost; on subsequent turns of continued diving it recieves 1.0 accel point per altitude level lost.
...
\begin{itemize}
    ...
    \itemaddedin{1B}{JDW: TSOH errata}{\itemparagraph{VD Prerequisites.} To enter a VD, the aircraft must have used diving flight the turn before (exception: the Half Roll and Dive maneuver allows VDs to be entered from level flight, see rule 13.3.5).}
    ...
    \begin{enumerate}
        \item[a)] When a steep or unloaded dive immediately follows a vertical one, half the aircraft's \changedin{1B}{JDW: TSOH errata}{speed}{FPs} (round down) must be VFPs or unloaded HFPs as appropriate.
...
\begin{itemize}
    ...
    \item Mate the half VFP to a previously carried half FP to create a full VFP \addedin{1B}{JDW: TSOH errata}{or HFP} and use it.

    \itemdeletedin{1B}{JDW: TSOH errata}{Steal a half VFP from the alloted HFPs to create a full VFP, and carry the remaining half HFP forward to the next turn.}
