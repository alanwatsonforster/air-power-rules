...
\paragraph{Internal DDS.} Internal DDS installations are identified on the ADC. Four types of internal DDS are available: A, B, C, D. \changedin{1D}{1D-tables}{The internal DDS Table}{Table~\ref{table:ew-internal-dds}} shows the standard loads of decoys available for an internal DDS but typically they are capable of holding between sixteen and twenty clusters of decoys.

\paragraph{External DDS.} External DDS pods are identified \changedin{1D}{1D-tables}{on the External Pod Table}{Table~\ref{table:stores-EP}}, which shows the possible loads of decoys available.
...
\paragraph{Decoy PPL Effects.} \changedin{1D}{1D-tables}{The various EW tables detail }{Tables~\ref{table:ew-decoy-ppl-effectiveness}}the specific effects or die roll modifiers PPLs produce. The following is a summary:
...
A barrage jammer has two modes: Noise and Stand-Off. The ability of an aircraft to switch between selected modes and jamming frequencies varies with the type of jammer and crew (pilot only or multi-crew) \changedin{1D}{1D-tables}{as detailed in the EW charts and EP tables}{is shown in Table~\ref{table:ew-bjm-flexibility}}. 
...
Subtract the ECCM rating of the jammed radar from the \x{barrage}{active} jammer rating and apply is as a die roll modifier for any search, passdowns, and/or lock-on attempts by the radar.
