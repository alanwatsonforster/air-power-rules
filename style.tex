...
\newcommand{\drawhex}[3][none]{
    \begin{athex}{#2}{#3}
        \begin{scope}[xscale=\hexxfactor,color=black!50]
...
\newcommand{\drawhexgrid}[3][0]{
    \clip (#1*\hexxfactor,0) rectangle (#2*\hexxfactor,#3);
    \foreach \x in {0,2,...,#2} {
        \foreach \y in {0,1,...,#3} {
...
\newcommand{\drawdotathex}[3][black]{
    \miniathex{#2}{#3}{\draw [fill=#1] circle (0.1);}
}
