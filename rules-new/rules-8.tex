...
\begin{itemize}
    ...
    \item\itemparagraph{ZC Procedure:} Declare climbing flight and note ZC as flight type on the Aircraft Log. At least 1/3 of FPs must be HFPs\changedin{1B}{1B-apj-21-qa}{; the remainder may be VFPs.}{. At least one and at most 2/3 of the FPs may be VFPs.}
...
Example of a SC with excess altitude gain: A MiG-21 with a speed of 6.0 in the LO band, CL configured, and at AB power has a CCC value of 4. It may have up to 4 VFPs and chooses to do so. Since the CCC value is greater than 2.0 it may gain two levels per VFP. The player elects to move as follows; H, H, \tightbinaryplus{V}{2}, \tightbinaryplus{V}{2}, \tightbinaryplus{V}{1}, \tightbinaryplus{V}{1} (moves forward two hexes and uses the four VFPs to gain 6 altitude levels). 0.5 Decel is incurred for each of the first four levels gained (the amount = to CCC value), and 1.0 decel for each of the last two (the amount exceeding CCC value). Total decel for climbing is 4.0.
...
Example: Consider a MiG-21 with a speed of 6.0 in the LO band, CL configured, and using afterburner power. It has a CC of 4.0. It may use up to 4 VFPs, and chooses to use all of them. Since its CC is more than 2.0, it may gain two levels per VFP. The player elects to move as follows; H, H, \tightbinaryplus{V}{2}, \tightbinaryplus{V}{2}, \tightbinaryplus{V}{1}, \tightbinaryplus{V}{1} (i.e., move forward two hexes and use the four VFPs to gain six altitude levels). The aircraft receives 0.5 DPs for each of the first four levels gained (up to the CC) and 1.0 DPs for each of the last two (in excess of the CC). It receives a total of 4.0 DPs for climbing.
}
...
Sustained climbs can result in an aircraft gaining a fraction of an altitude level. Such fractional gains are added to any that have been carried into the current game turn. If the sum is one or more, the aircraft gains a full altitude level and the remaining fraction is carried forward into the next game turn. If the sum is less than one, it is carried forward into the next game turn.

Fractional gains are carried forward as long as an aircraft continues to use climbing flight. Fractional gains are lost if an aircraft chooses level or diving flight.

An aircraft only receives DPs for each full level gained. It does not receive DPs for a fractional altitude gain.

For all other purposes, fractional gains are ignored. That is, for all other purposes, a climbing aircraft's altitude is always the last full level gained.

For example, consider an aircraft with an altitude of 10, a CC of 0.25, and initially no fractional climb carry. If performs a sustained climb over several game turns. Each game turn it receives one VFP that allows it to gain 0.25 levels. It will carry 0.25 levels into the second game turn, 0.50 into the third, and 0.75 into the fourth. On the first, second, and third game turns it will receive no DPs for climbing. In the fourth game turn, the 0.25 levels gained by the VFP will combine with the 0.75 levels carried in to give a gain of one full level. The aircraft will now have an altitude of 11 and will receive 0.5 DPs. 
...
\begin{itemize}
    ...
    \item\itemparagraph{SD Procedure:} Declare diving flight and note SD as flight type on the aircraft log. At least 1/3 of FPs must be HFPs. \changedin{1B}{1B-apj-21-qa}{The rest may be VFPs.}{At least one and at most 2/3 of the FPs may be VFPs.}
...
\begin{itemize}
    ...
    \itemaddedin{1B}{1B-apj-21-qa/1B-apj-22-qa/1B-apj-23-errata}{\itemparagraph{VD Prerequisites.} To enter a VD, the aircraft must have used diving flight the turn before (exception: the Half Roll and Dive maneuver allows VDs to be entered from level flight, see rule 13.3.5).}
