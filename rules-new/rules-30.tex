...
The flight conditions are: whether it is day or night; the position of the Sun during the day; and the presence and altitudes of contrail, haze, and cloud layers.
...
\AX{
\paragraph{Day or Night.}

Determine whether the scenario occurs at night by rolling one die and considering the scenario year. The scenario occurs at night on a roll of \minusafter{1} in the 1950s, \minusafter{2} in the 1960s, \minusafter{3} in the 1970s or 1980s, and \minusafter{4} in the 1990s or later.
...
The altitudes of each haze layer or cloud layer are determined as follows:
...
IR missiles launched in target's Sun Clutter add 3 to the launch roll. IR missiles ending a proportional move or game-turn in target's Sun Clutter must roll a die to see if they are decoyed by the Sun. Use the Flare Vulnerability number \plus{3} as the die roll or less needed.
...
IRMs and IR SAMs launched from within their target’s Sun clutter have a \plus{3} modifier to their launch roll (see rule \ref{rule:irm-launch-modifiers}).
...
\paragraph{AAA Aimed Fire Modifier.} AAA units firing on the target from within its Sun clutter  suffer a \plus{1} modifier to their hit die roll for aimed fire, unless they are stacked with a FCR or have an integral W-type FCR.
...
Contrails dramatically increase the visibility of the aircraft or missile generating them.

\paragraph{Contrails.} If an aircraft or missile is at a speed of 4.0 or more and within the contrail layer, it generates a contrail.
...
\item At night, a contrailing aircraft or missile that is \emph{not above the highest cloud layer present} can be automatically sighted to a sighting range of 6.
\end{itemize}
...
Haze layers can absorb or scatter visible light, which reduces contrast and has a negative impact on visual sighting and on sensors or weapons that operate with visible light. On the other hand, in the game, their impact on infrared light and radar is negligible. The precise effects are the following.

\CY[3A-sighting]{
\paragraph{Maximum Sighting Range.} If either the searcher or target or both are in a haze layer,  the maximum sighting range for an aircraft is twice its visibility number.
}{
\paragraph{Sighting Range.} If either the searcher or target or both are in a haze layer, the sighting range for visual sighting and identification (see rule~\ref{rule:visual-sighting}) is considered to be twice the normal range, which reduces both the maximum range to which the target can be sighted and gives more adverse modifiers. This applies to normal visual sighting (air-to-air, air-to-ground, and ground-to-air) and to the use of VAS. It does not apply to the use of IRSTS or TV/IR optics.
}

\DY[3A-sighting]{
\itemparagraph{Sighting Roll Modifiers.} If the target is in haze, use the sighting modifier from Table \ref{table:sighting-position-modifiers} in addition to the normal position modifier.
}
...
Exceptionally and for this purpose only, if a potential target moves first and enters a haze layer, \CY[3A-sighting]{the maximum sighting range is considered to be the normal value of four times the visibility number and not twice the visibility number}{the sighting range is considered to be the actual range and not twice the actual range}. This gives an attacker the ability to follow and attack a target that tries to escape by entering a haze layer.
...
\item A target aircraft and missile in a stratus cloud layer can be sighted by other aircraft with a modifier of \plus{3}. 

\item An aircraft in a stratus cloud layer can sight other aircraft and missiles with a modifier of \plus{2}.

\IAY[3A-sighting]{If the target is below the highest dense cloud layer, a \plus{1} modifier applies (reflecting the poorer light below dense clouds).}
...
The scenario notes may indicate night conditions. If not, they may be agreed on by the players or determined randomly following this procedure. Roll one die. The scenario occurs at night on a roll of \minusafter{1} for scenarios in the 1950s, \minusafter{2} in the 1960s, \minusafter{3} in the 1970s or 1980s, and \minusafter{4} in the 1990s or later.
}
...
\begin{itemize}
    ...
    \itemdeletedin{2B}{2B-contrails-at-night}{Visual sighting ranges for all aircraft is 2 hexes unless the target \changedin{2B}{2B-contrails-at-night}{used AB power or is contrailing}{} in which case the sighting range is 6. Sighting modifiers for paint scheme, relative altitude and smoking do not apply at night.}

    \itemaddedin{2B}{2B-contrails-at-night}{Unless they are contrailing, visual sighting ranges for all aircraft is 2 hexes unless the target used AB power in which case the sighting range is 6. Sighting modifiers for paint scheme, relative altitude and smoking do not apply at night.}

    \itemdeletedin{2B}{2B-contrails-at-night}{Missiles may only be sighted on the turn after launch and on each turn a sustainer is burning. The visibility numbers are halved and the Just Launched modifier does not apply.}
    
    \itemaddedin{2B}{2B-contrails-at-night}{Unless they are contrailing, missiles may only be sighted on the turn after launch and on each turn a sustainer is burning. The visibility numbers are halved and the Just Launched modifier does not apply.}
    ...
    \itemaddedin{2B}{2B-contrails-at-night}{Missiles or aircraft contrailing not above the highest cloud layer are spotted automatically up to 6 hexes away.}
    ...
    \itemaddedin{2A}{2A-identifying-at-night} Identifying an aircraft at night requires that the searcher have the same position, facing, and speed as the target.
...
\begin{itemize}
    ...
    \item \plus{1} for Veteran Pilots.
    \item \minus{1} per facing in a sustained turn (cumulative).
\end{itemize}
...
\begin{itemize}
    ...
    \item\itemparagraph{TFF.} TFF is not allowed unless the aircraft has Terrain Following technology or TV/IR optics technology (see advanced rule \ref{rule:tv-ir-optics}).
...
\begin{itemize}
    ...
    \item\itemparagraph{Maneuvers.} All special maneuvers except slides not allowed.
...
\item
Sighting modifiers for \CY[3A-sighting]{paint scheme, relative altitude,}{background} and smoking do not apply at night.
\end{itemize}
...
\paragraph{Identifying Aircraft.} Identifying an aircraft at night requires that the searcher have the same position, facing, and speed as the target.
...
\paragraph{IR/OG/LG SAMs.} An IR/OG/LG SAM unit may only attempt lock-ons or launches on targets within the reduced maximum visual sighting ranges given above. 
...
\item It has an air-to-air radar with an arc of \arcrange{150}{+}.
\item It is a multi-crewed aircraft and has an air-to-air radar with an arc of \arcrange{180}{+}.
\end{itemize}
