...
This rule concerns the position and facing of an element --- an aircraft, a missile, a ground unit, or a naval unit --- and the range, the horizontal arc, and the vertical arc of one element with respect to another. 
...
\begin{itemize}
    \IAX{If \CY[3A-head-on-attacks]{an}{a moving or non-moving} aircraft carries out a head-on gun attack at range 0 (see rule~\ref{rule:air-to-air-gun-combat}) on a target at the same altitude.}
    \item \CX{
...
For example, if an aircraft executes level flight with four HFPs and no free descent, its flight slope is 0. If it executes the same but with free descent, its flight slope is \minus{1/4 = \minus{0}.25}. If it executes a zoom climb with three HFPs and four VFPs, gaining seven altitude levels, its flight slope is \plus{7/3 \approx 2.33}. If it executes a vertical dive with no HFPs and six VFPs, losing twelve altitude levels, its flight slope is \minus{12/0 = -\infinity}.
