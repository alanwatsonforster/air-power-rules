...
\begin{itemize}
    ...
    \item In combat: Gun attacks, visual bombing or rocket attacks, \changedin{1B}{1B-apj-22-qa/1B-apj-39-qa}{and IRM launches}{IRM launches, and the use of auto-track and boresight radar modes}.
\end{itemize}
...
\begin{itemize}
    ...
    The results of the rolls for characteristics depend on the crew quality, and the tactics master roll is modified by \minus{1} for excellent training standard and \plus{1} for limited or poor training standard. 
...
\label{rule:crew-ability-effects}
...
When an aircraft turns too sharply, crewmembers may lose consciousness due to G-force effects. This is known as G-induced loss of consciousness or GLOC.


\paragraph{When Can GLOC Occur?} 
GLOC can occur when an aircraft changes facing at the ET rate in the LO, ML, or MH altitude bands. Each crewmember must check for GLOC after each facing change at the ET rate in these bands. At higher altitudes, the G-force of an ET turn is insufficient to cause GLOC.

\input{tables/table-gloc}

\paragraph{GLOC Procedure.} 
Roll the die once for each crewmember and apply any required modifiers\addedin{1C}{1C-tables}{\ from Table~\ref{table:gloc-avoidance}}. A result of \minusafter{1} results in GLOC. 
...
\begin{itemize}
    ...
    \item\itemparagraph{Unconscious Crewmember.} An unconscious crewmember may not padlock or participate in visual sighting (see rule \ref{rule:visual-sighting}) and may not eject themselves or bale out.

    \item\itemparagraph{Unconscious Pilot.} The aircraft's flight is randomized and controlled by Table~\ref{table:gloc-flight}. If the pilot becomes unconscious when the aircraft still has FPs to use, roll on the table to finish the aircraft's move. Roll the die on subsequent game turns until the pilot recovers or is ejected or the aircraft is destroyed.

    \item\itemparagraph{Unconscious Non-Pilot Crewmember.} The efforts of that crewmember are lost until he recovers. If the unconscious crewmember is normally the radar operator, the radar cannot be used except in auto-track and boresight modes. The bombsight is degraded by one level (but never to less than manual). Multi-crew sighting and any other multi-crew benefits are lost.

    \item\itemparagraph{Ejection.} If the pilot of a multi-crew aircraft suffers GLOC, and randomized flight of the aircraft will result in an inevitable crash, a conscious crewmember can eject themselves and all other crewmembers.
\end{itemize}
}
...
\input{tables/table-disorientation}

Pilots can become disoriented. Green and novice pilots risk disorientation if they perform certain maneuvers or select certain flight types, as described in advanced rule~\ref{rule:crew-ability}. Any pilot can become disoriented in adverse weather or at night, according to advanced rule~\ref{rule:flying-at-night}.

\paragraph{Pilot Disorientation.} Anytime an aircraft performs an action which risks disorientation, roll one die and apply appropriate modifiers from Table~\ref{table:disorientation-avoidance}. If the result is \minusafter{3}, the pilot becomes disoriented.
...
A pilot may attempt to recover from disorientation in the stalled aircraft and disoriented pilot phase. Roll one die and apply appropriate modifiers from Table~\ref{table:disorientation-recovery}. If the result is \minusafter{6}, the pilot recovers from disorientation and can perform normally.
...
\begin{itemize}
    ...
    \item \itemparagraph{Green to Novice.} After three or more combat missions or one or more air-to-air kills and rolling \minusafter{8}.

    \item \itemparagraph{Novice to Regular.} After five or more combat missions as a novice or one or more air-to-air kills as a novice and rolling \minusafter{6}.

    \item \itemparagraph{Regular to Veteran.} After five or more combat missions as a regular or one or more air-to-air kills as a regular and rolling \minusafter{4}.
...
\begin{itemize}
    ...
    \item \itemparagraph{Combat Hero.} After gaining five or more air-to-air kills and rolling \minusafter{9}.

    % ISSUE: Is the roll performed once? Or each time they gain a kill beyond four?

    \item \itemparagraph{Decorated Combat Hero.} Upon rolling \minusafter{2} after any single mission in which the players collectively feel the crewmember in question performed in an extraordinary manner that deserves a medal. This requirement is vague, but it usually is in real life, too.

    \item \itemparagraph{Tactics Master.} Upon improving to regular or veteran quality and rolling \minusafter{2}. This roll is performed once for each of these quality improvements, and if missed after achieving veteran status, it is never attained.

    \item \itemparagraph{Sierra Hotel.} Upon improving in quality to any level and rolling a \minusafter{1}. This roll is performed once for each of these quality improvements.
