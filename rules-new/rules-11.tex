...
This rule details the procedures for visual sighting and identification of targets during daylight and in clear weather. 

Advanced rule~\ref{rule:night-and-weather} covers the modifications appropriate for sighting at night and in the presence of the Sun, clouds, and haze.
...
Aircraft and missiles are either \emph{sighted} or \emph{unsighted} by opposing aircraft. This is important for many reasons. For example, aircraft may only fire their guns at sighted target aircraft and may only defensively engage sighted missiles.

\paragraph{General Procedure.} 
Sighting is determined in the visual sighting phase.
Each side first declares padlocks of currently sighted opposing aircraft and missiles. Any opposing aircraft or missile that is not padlocked is considered to become unsighted. Each side can then attempt to sight first unsighted opposing missiles and then unsighted opposing aircraft. If an attempt is successful, the missile or aircraft becomes sighted. At the end of the visual sighting phase, the only opposing aircraft and missiles that are sighted are those that were padlocked or for which the sighting attempt succeeded. All other opposing aircraft and missiles are unsighted.

\paragraph{Duration of Sighting.}
An aircraft's or missile's status as sighted or unsighted is determined anew in the visual sighting phase of each game turn. It then maintains that status until the next visual sighting phase. A sighted aircraft or missile that enters a blind arc does not immediately cease to be sighted; it remains sighted at least until the next visual sighting phase.
...
\paragraph{Visual Sighting Procedure.} In the Visual Sighting Procedure Phase, the friendly aircraft which is closest to\addedin{1B}{1B-apj-23-errata/1B-apj-35-qa}{, and not blind to or blocked by clouds,} an unsighted or unpadlocked enemy aircraft or missile is used as the reference aircraft for sighting attempts against it. If more than one friendly aircraft is the same distance from an enemy, the searching player may designate which of the friendly aircraft will be the searcher. \addedin{1B}{1B-apj-23-errata/1B-apj-35-qa}{All aircraft on a side within sighting range \addedin{1B}{1B-apj-25-qa}{of and not blind to an enemy }may participate in the sighting attempt of that enemy even if they were used to padlock other targets in the same sighting phase.}
...
\paragraph{Sighting Range.} The range for sighting is determined according to rule~\ref{rule:range}. When attempting to sight an aircraft or missile that is \emph{higher}, the vertical range is the difference in altitude levels divided by \emph{four} and rounded down, as it is easier to see an aircraft or missile silhouetted against the sky. When attempting to sight an aircraft or missile or one that is lower or level, the vertical range is the difference in altitude levels divided by two, as usual, and rounded down.

\AY[3A-sighting-weather]{\relevantadvancedrules{\ref{rule:haze-layers}.}}

\paragraph{Visibility Number.} An aircraft or a missile in flight has a visibility number, given for aircraft by their ADC and for missiles by the Missile Data Table (Table~\ref{table:missile-data}). The visibility number determines the maximum range for sighting.

\paragraph{Maximum Sighting Range.} During daylight and in clear weather, the maximum visual sighting range to an aircraft or missile is four times its visibility number. Table \ref{table:maximum-sighting-range} summarizes the maximum sighting range.

\relevantadvancedrules{\ref{rule:vas}, \ref{rule:irsts}, \ref{rule:limited-intelligence}, \ref{rule:contrail-layers}, \ref{rule:haze-layers}, and~\ref{rule:combat-at-night}.}
...
An aircraft cannot padlock or attempt to sight a target in its blind arcs. 

An aircraft can padlock and attempt to sight a target in its restricted arcs, but receives an adverse modifier to the sighting die roll.

\AY[3A-horizontal-arcs]{
When determining the blind and restricted horizontal arcs (see rule \ref{rule:horizontal-arcs}), the aircraft attempting sighting is the reference element and the target aircraft or missile being sighted is the distant element. Resolve borderline cases by first moving the faster element forward. If the faster element remains on the borderline or the two elements have the same speed, use the rearward arc. For example, if the faster element remains on the borderline between the \arc{60} and \arc{90} arcs, use the \arc{60} arc.
}

An aircraft cannot attempt to sight a target at the same map location (i.e., the same hex or hex side) and lower altitude, but can padlock such a target (as it can roll slightly to keep the target in view). An aircraft may sight or padlock targets at the same map location but higher altitude.
...
\relevantadvancedrules{\ref{rule:gloc}, \ref{rule:disorientation}, and \ref{rule:sun}.}
...
For each unsighted opposing target, identify which friendly aircraft can participate in the sighting attempt. To participate, an aircraft must be within maximum visual sighting range of the target, must not have the target in its blind arc, and must have an unblocked line of sight. 

If no friendly aircraft can participate, the unsighted target remains unsighted.

\CY[3A-sighting]{Otherwise, determine which of the participating aircraft is the searcher. This will normally be the participating friendly aircraft closest to the target, but if two or more are equally close, their players may designate which is the searcher.}{Otherwise, the player may designate which of the participating aircraft is the searcher. This will often be the participating friendly aircraft closest to the target, but sometimes it is worthwhile to designate a different aircraft with more advantageous modifiers.} Roll a die and apply appropriate modifiers. The target is sighted if the modified result is less than or equal to the target's visibility number.
...
\begin{itemize}
    ...
    \IDY[3A-sighting]{\itemparagraph{Target Position.} Apply the appropriate modifiers from Table~\ref{table:sighting-position-modifiers} for altitude of the target aircraft relative to the searcher. These modifiers depend on the paint scheme of the target and do not apply to missiles.}
    
    \IAY[3A-sighting]{\itemparagraph{Target Position.} Apply the appropriate modifier from Table~\ref{table:sighting-position-modifiers} for the position of the target aircraft with respect to the searcher:
    \begin{itemize}
    \item\itemparagraph{Against Land.} This modifier applies if the target is below the searcher and on any map sheet that in which the majority of hexes are not water hexes.
    \item\itemparagraph{Against Water.} This modifier applies if the target is below the searcher and on a map sheet in which the majority of hexes are water hexes.
    ...
    \end{itemize}   
    The target position modifiers do not apply when sighting missiles.
    }
    ...
    To participate, an aircraft must be within maximum visual sighting range of the target, must not have the target in its blind arc, and must have an unblocked line of sight.

    \item \itemparagraph{Restricted Arc.} If the target is in the restricted arcs of \CY[3A-sighting]{\emph{all} of the participants}{the searcher}, apply a \plus{2} modifier.
    \item \itemparagraph{Target is Smoker.} 
    \CY[3A-sighting]{If the target is a smoker, apply the modifiers from Table~\ref{table:sighting-position-modifiers} for altitude of the target relative to the searcher.}{If a target aircraft is a smoker and lower than the searcher, apply a \minus{1} modifier. If it is a smoker and level with or higher than the searcher, apply a \minus{2} modifier.} 
    
    Aircraft can be smokers in two different circumstances. Some aircraft are noted on their power charts as being smokers when they use military power. Also, some results on the optional damage tables cause an aircraft to become a smoker regardless of their power setting.
    \item \itemparagraph{Missile Motor.} If the target is a missile whose boost or sustainer motor burned in the previous game turn, apply a \minus{3} modifier for a normal motor and a \minus{1} modifier for a smokeless motor.
    \item \itemparagraph{Missile Launch.} If the target is a missile launched from a sighted aircraft in the previous game turn or an aircraft that launched a sighted missile in the previous game turn, apply a \minus{2} modifier. (For these modifiers, it is important to attempt to sight missiles before aircraft.)
    \item \itemparagraph{Searcher is Multi-Crew.} If the searcher is a multi-crew aircraft, apply a \minus{1} modifier.
    \item\itemparagraph{Searcher has Detection.} If the searcher has a radar detection (or lock-on) of the target, apply a \minus{1} modifier.

\end{itemize}

\relevantadvancedrules{\ref{rule:hud}, \ref{rule:irsts}, \ref{rule:crew-ability-effects}, \ref{rule:dds}, \ref{rule:rwr}, \ref{rule:haze-layers}, and~\ref{rule:cloud-layers}.}

\paragraph{Aircraft Paint Schemes.}
The target position modifiers depend on the paint scheme of the target aircraft:
\begin{itemize}
\item\itemparagraph{Silver.} This corresponds aircraft in a natural-metal finish that produces glints.
\IDY[3A-sighting]{\itemparagraph{Uncamouflaged.} This corresponds to aircraft with an uncamouflaged paint scheme in light colors (for example, USN aircraft in Light Gull Grey and Ivory White in the 1960s) or dark gray or blue (for example, USN aircraft in Glossy Sea Blue in the early 1950s).}
\IAY[3A-sighting]{\itemparagraph{Light Uncamouflaged.} This corresponds to aircraft with an uncamouflaged paint scheme in light colors (for example, USN aircraft in Light Gull Grey and Ivory White in the 1960s).}
\IAY[3A-sighting]{\itemparagraph{Dark Uncamouflaged.} This corresponds to aircraft with an uncamouflaged paint scheme in dark gray or blue (for example, USN aircraft in Glossy Sea Blue in the early 1950s).}
...
\end{itemize}
The scenario notes will indicate the paint schemes of the aircraft. If an aircraft has an inappropriate camouflaged paint scheme for the zone (for example, a desert camouflage scheme in a temperate zone), the scenario notes may indicate that the aircraft should be use the \CY[3A-sighting]{uncamouflaged}{light or dark uncamouflaged} modifiers when seen against land.
}
...
Each pilot only crewed friendly aircraft may padlock target. Each multi-crew aircraft may padlock two targets instead of one. Crew quality (see Chapter 18) may also affect how many padlocks are possible.
...
\AX{
\section{HUD Interface Technology}
\label{rule:hud}

Aircraft with \emph{HUD interface} technology have their heads-up displays (HUDs) interfaced to their radar or IRSTS systems. This allows the HUD to indicate to the pilot the position of a target aircraft being tracked and makes subsequent sighting easier.

\paragraph{Sighting Effects.}
If an aircraft has HUD interface technology and has a radar or IRSTS lock-on (see rule \ref{rule:air-to-air-radar} and \ref{rule:irsts}) on a target aircraft in their \arcrange{180}{+} \CY[3A-combined-arcs]{arcs}{combined arcs (see rule \ref{rule:combined-arcs})}, they gain the following sighting benefits benefits:

\begin{itemize}

\item\itemparagraph{Sighting Roll Modifier.}
Apply a \minus{1} modifier to the sighting roll, in addition to the \minus{1} modifier for having a detection or track. This modifier is applied only once regardless of how many different types of lock-ons the searcher has for the target.

\item\itemparagraph{Maximum Sighting Range.} The maximum sighting range at night is doubled (see advanced rule \ref{rule:combat-at-night}).

\end{itemize}

These benefits do not apply to VAS searches (see rule \ref{rule:vas}).
}
...
\paragraph{VAS Considerations.} Enemy aircraft sighted by VAS systems are sighted only to the VAS equipped aircraft that specifically saw them and may not be attacked by other aircraft unless also visually sighted by the regular procedure. VAS systems may be used to accomplish target identification. \changedin{2B}{2B-vas-and-padlocks}{An aircraft}{A pilot} that elects to use VAS during the Visual Sighting Phase may not padlock.
...
A pilot that elects to use VAS during the visual sighting phase may not padlock. An aircraft sighted by a VAS system is sighted only to the VAS-equipped aircraft that sighted it and may only be attacked by other aircraft if it is also visually sighted normally.
...
\begin{itemize}
    ...
    \item On a roll of \minusafter{7}, when its range to a VAS-equipped friendly aircraft is no more than 10 times its visibility number, and it is in the \arcrange{180}{+} \AY[3A-combined-arcs]{combined} arc of that aircraft.
...
\begin{itemize}
    ...
    \item The missile seeker type, if decoys defeat it.
    \item The missile seeker type, if it hits.
\end{itemize}
