...
\begin{itemize}
    ...
    \CY[3A-sighting]{If the target is a smoker, apply the modifiers from Table~\ref{table:sighting-position-modifiers} for altitude of the target relative to the searcher.}{If a target aircraft is a smoker and lower than the searcher, apply a $-1$ modifier. If it is a smoker and level with or higher than the searcher, apply a $-2$ modifier.} 
    ...
    \item \itemparagraph{Searcher is Multi-Crew.} If the searcher is a multi-crew aircraft, apply a $-1$ modifier.
    \item\itemparagraph{Searcher has Detection.} If the searcher has a radar detection (or lock-on) of the target, apply a $-1$ modifier.
...
Apply a $-1$ modifier to the sighting roll, in addition to the $-1$ modifier for having a detection or track. This modifier is applied only once regardless of how many different types of lock-ons the searcher has for the target.
