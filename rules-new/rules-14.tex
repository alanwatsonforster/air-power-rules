...
Targets may defend themselves against air-to-air missiles by maneuvering so that they can no longer be tracked by the seeker (see rules~\ref{rule:infrared-heat-seeking-missiles} and \ref{rule:radar-guided-missiles}) by dispensing decoys such as flares, chaff, and mini-jammers (see advanced rule~\ref{rule:dds}), by using radar or infrared jammers (see advanced rules~\ref{rule:radar-jamming} and \ref{rule:infrared-jammers}).
}
...
\begin{itemize}
    ...
    \item \itemparagraph{Launch Turn Rate.} This is the maximum turn rate an aircraft may use during flight to launch the missile without penalty. If the aircraft used a higher rate, a cumulative \plus{2} modifier applies to the launch roll for each level of higher turn rate.
    ...
    \item \itemparagraph{Hit Roll.} The two columns list the number or less for the missile’s attack hit roll that will result in a direct (first column) or proximity (second column) hit. A direct hit gives a \minus{2} damage roll modifier.
...
\begin{itemize}
    ...
    Each missile has a maximum launch turn rate given in Table~\ref{table:missile-data}. If the launching aircraft has used a higher turn rate and has not yet completed the corresponding recovery period (see rule~\ref{rule:recovery-periods}), it suffers a \plus{2} modifier for each turn rate above the maximum.

    For example, if a missile has a maximum launch turn rate of TT and the launching aircraft used a BT turn rate and has not yet completed the recovery period, a \plus{4} modifier would apply to the launch roll.

    \item If a missile is launched out-of-envelope, it receives a \plus{3} modifier (see advanced rules \ref{rule:irm-out-of-envelope-launches} and \ref{rule:radar-out-of-envelope-launches}).
...
\begin{itemize}
    ...
    \item \minus{1} if the speed of the launch aircraft on the turn after launch is 3.0 or less,

    \item \plus{1} if the speed is 6.0 or more, and

    \item \plus{2} if the speed is 9.0 or more.
\end{itemize}
...
\begin{itemize}
    ...
    \item \minus{1} if the missile's altitude gain for the game turn in levels is half or more of its speed or \minus{2} if it is its speed or more.

    \item \plus{1} if the missile's altitude loss for the game turn in levels is half or more of its speed or \plus{2} if it is its speed or more.
...
If the proportion includes leftover FPs, for example, say the missile's speed was 17.0 instead of 15.0. In this case five goes into seventeen three times with two left over. The missile would still move three for one except the two extra points are added on one at a time in the first two segments of movement that turn. That is, \changedin{2A}{2A-missile-flight}{the aircraft would move one, then the missile four (\binaryplus{3}{1}), then the aircraft one and missile four again}{the missile would move four FPs, then the aircraft one, then the missile four, and the the aircraft one again}. After that, the missile reverts to its three to one proportion.
...
For example, consider an attack by an AIM-9B Sidewinder missile. Table~\ref{table:missile-data} gives hit die rolls of \minusafter{3} for a direct hit and \minusafter{7} for a proximity hit. The missile achieves a direct hit if the modified hit die roll is 3 or less. The missile achieves a proximity hit if the modified hit die roll is 4 to 7. The missile misses if the modified hit die roll is 8 or more.
}
...
\begin{itemize}
    ...
    \item If the target did not engage the missile, apply a modifier of \minus{1}.
...
\paragraph{Missile Damage.} Table~\ref{table:missile-data} gives the attack rating of the missile for direct and proximity hits. The ratings are used to determine aircraft damage according to rule~\ref{rule:aircraft-damage-resolution}. However, a direct hit always gets a \minus{2} modifier to the damage die roll to account for the additional effects of the warhead blast above the shrapnel from a proximity hit.

For example, consider an attack by an AIM-9B Sidewinder missile. Table~\ref{table:missile-data} gives attack ratings of 5 for a direct hit and 2 for a proximity hit. If the missile achieves a direct hit, damage is determined with an attack rating of 5 and an additional die roll modifier of \minus{2}. If the missile achieves a proximity hit, damage is determined with an attack rating of 2 without the additional die roll modifier.
}
...
\begin{itemize}
    ...
    \item If it selects idle power, it automatically gains the \plus{1} hit modifier against attacks by IR missiles. This benefit applies to all IR missile attacks, even those by unsighted missiles or missiles for which it has not been alerted by ECM.
...
Against engaged aircraft, missiles that were not sighted or for which it has not been alerted are still affected by idle power and do not get the \minus{1} to hit modifier.
...
\begin{itemize}
    ...
    \item It suffers a \minus{1} hit modifier in missile attacks.

    % ISSUE: roll for each missile or roll once per turn?
    \item If it selects idle power, it only gains the \plus{1} hit modifier on a roll of \minusafter{4}. 
