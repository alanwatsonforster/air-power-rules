...
\begin{itemize}
\item The horizontal range is determined using Figure~\ref{figure:air-to-air-gun-attacks}. If the target occupies the same map location (i.e., the same hex or hex side) as the attacker, the horizontal range is 0.
...
\begin{itemize}
    ...
    \item An aircraft in level flight may fire on a target in the same map location  (i.e., the same hex or hex side) only if it is at the same altitude.
    \item An aircraft in level flight may fire on a target in another map location  (i.e., a different hex or hex side) only if it is at the same or an adjacent altitude level.
...
\begin{itemize}
    ...
    If the attack is a snap shot, apply a modifier of \plus{1}.
    }

    \CY[3A-horizontal-arcs]{
    \CX{
    ...
    \item\itemparagraph{Horizontal Arc.}
    If the attacker is not immediately behind or in front of the target, they must compensate for the \emph{deflection} or lateral motion of the target during the flight time of the bullets or shells. This becomes harder as the angle-off gets closer to the beam. Determine the horizontal arc of the attacker from the target (see rule~\ref{rule:angle-off}) and apply the modifiers given in Table~\ref{table:air-to-air-gun-and-rocket-modifiers}.
    
    When determining the horizontal arc, the target is the reference element and the attacker is the distant element. Resolve borderline cases by first moving the faster aircraft forward. If the faster aircraft remains on the borderline, use the narrower arc.
    }
    ...
    \CY[3A-vertical-attacks]{
    A climbing or diving aircraft firing at a target in the same map location  (i.e., the same hex or hex side) may be subject to a vertical-attack modifier. 
    
    If the attacker is climbing and the target is diving or if the attacker is diving and the target is climbing, the modifier is \plus{2}. 

    If the attacker is climbing or diving and the target is in level flight\AY[3A-vertical-attacks]{ or if the attacker is in level flight and the target is climbing or diving}, the modifier is \plus{1}.
    
    If both the attacker and target are both in level flight, both climbing, or both diving, there is no modifier.
    }{
    If an aircraft fires at a target in the same map location (i.e., the same hex or hex side) may be subject to a vertical-attack modifier. 
    
    If the attacker is climbing and the target is diving or if the attacker is diving and the target is climbing, the modifier is \plus{2}. 
    
    If the attacker is climbing or diving and the target is in level flight or if the attacker is in level flight and the target is climbing or diving, the modifier is \plus{1}. 
    
    If both the attacker and target are both in level flight, both climbing, or both diving, there is no modifier.
    }
    
    For aircraft that have not yet moved, use the flight type from the previous turn to determine the modifier.
...
paragraph{Head-On Gun Attacks.} 
\CY[3A-head-on-gun-attacks]{
If both the attacker and target directly face each other with the attacker on the \arc{180} line shown in Figure~\ref{figure:angle-off}, the attack is a head-on attack. A target aircraft waiting to move (or that has already finished its move) may return fire in response to head-on attacks provided it does not exceed the two-shot limit per game turn and satisfies all of the normal requirements except the sighting requirement. An aircraft that has not flown yet may \emph{not} respond to a head-on attack if it is carrying or recovering from an ET turn or prepared for or executed a rolling maneuver on the last FP of the previous game turn. Gunsight modifiers apply if it is carrying a turn or recovering from a turn. \addedin{2B}{2B-head-on-gun-attacks}{If the target returns fire, both aircraft roll to hit before any damage from the attack or response is applied. If the target has not yet moved in this game turn, its flight type from the previous game turn is used to determine vertical restrictions (e.g., that a diving aircraft may not fire on a higher aircraft) and for determining the same-location vertical attack modifier.}
}{
\paragraph{Head-On Gun Attacks.} 
If a moving and non-moving aircraft (i.e., one that has already moved or has not yet moved this game turn) directly face each other on the \arc{180} line and are within gun range, they may each carry out head-on gun attacks. After each FP, the moving aircraft may declare a gun attack and the non-moving aircraft may then declare a gun attack. The non-moving aircraft can declare an attack even if the moving aircraft does not.

If both aircraft declare gun attacks, both roll to hit before any damage from the attacks is applied.

The moving aircraft must not exceed the two-shot limit per game turn and must satisfy all of the normal requirements. 

The non-moving aircraft must not exceed the two-shot limit per game turn and must satisfy all of the normal requirements. However, if the moving aircraft declares a head-on gun attack, the non-moving aircraft can return fire even if the moving aircraft is unsighted or not individually sighted by the non-moving aircraft. The non-moving aircraft may \emph{not} declare a head-on attack if it is carrying or recovering from an ET turn or prepared for or executed a rolling maneuver on the last FP of its previous move. Gunsight modifiers apply if it is carrying a turn or recovering from a turn. 

If the non-moving aircraft has not yet moved in this game turn, its flight type from the previous game turn is used to determine vertical restrictions (e.g., that a diving aircraft may not fire on a higher aircraft) and for determining the same-location vertical attack modifier.
...
If the aircraft is firing internal guns and pods or more than one pod, a single die roll is used for the attack. The roll is compared to the hit die rolls of each gun firing. The damage rating is the highest of the guns that hit and is increased by \plus{1} for each additional gun that hits.
}

\DX{
\addedin{1B}{1B-apj-23-errata}{\paragraph{Vertical Attack Modifier.} 
The same hex \plus{2} Vertical Attack Modifier only applies if the target uses climbing or diving flight and the firer uses the opposite (i.e., diving aircraft attacking a climbing aircraft). A diving attacker firing on a diving target and a climbing attacker firing on a climbing target in the same hex are not subject to the vertical attack modifier. A climbing or diving attacker firing on a level target is subject to a reduced Vertical Attack Modifier of \plus{1}.}\addedin{2B}{2B-vertical-attacks}{ For aircraft that have not yet moved, use the flight type from the previous turn to determine the modifier.}
}
...
\paragraph{Angle-Off Arcs.} Angle-off is described in \arc{30} arcs to the left or right of the target. Figure~\ref{figure:angle-off} show these arcs. The appropriate figure is used according to whether the target aircraft is in a hex facing a hex corner, in a hex facing a hex side, or on a hex side.
...
For example, consider Figure~\ref{figure:angle-off}, in which the aircraft A1, A2, and A3 are targets and the aircraft B1, B2,  B3, C1, C2, and C3 are attackers. Aircraft C2 is unambiguously on the \arc{0} line, and C1 is unambiguously in the right \arc{90} arc, but the other attackers are on borderlines between two arcs. Aircraft C3 cannot be on the \arc{0} line as it is not facing the target.

\begin{itemize}

\item
If the targets were slower than the attackers, then B1 would be in the adjacent arc (right \arc{60} or \arc{90} arc) that would give it the more beneficial modifier, B2 would be in the right \arc{150} arc, B3 would be in the left \arc{120} arc, and C3 in the left \arc{30} arc. 

\item
On the other hand, if the targets were faster than the attackers, B1 would be in the right \arc{60} arc, B3 would be in the left \arc{90} arc, and the other arcs would be unchanged. 
...
\begin{itemize}
    \item \itemparagraph{CC Rocket Attack.} If the attacker satisfies the requirements for a CC rocket attack, apply a modifier of \minus{2}.
\end{itemize}
...
\end{itemize}
If it meets these criteria, apply a \minus{2} modifier to the hit die roll.
}
...
\paragraph{SSGT Procedure.}
 To benefit from SSGT, an aircraft must accumulate FPs consecutively while on a tracking line (see Figure~\ref{figure:ssgt-lines}), at a range of no more than six hexes from the target, and with the target in the field of fire of its guns or in its limited horizontal arc. Both VFPs and HFPs count for SSGT. FPs used for an ET turn, unloaded FPs, or FPs used to prepare for or execute a rolling maneuver do not count for SSGT, and neither do FPs used for the recovery periods after an ET turn or an unloaded FP. FPs used for or to recover from turns at lesser turn rates do count. Attacking or turning during a move does not interrupt the accumulation of FPs.

\paragraph{SSGT Modifiers.} When attacking with guns or rockets, an aircraft gains a \minus{1} modifier of each 1/3 of its speed (round down) in accumulated FPs. The maximum modifier is \minus{2}. 
...
\item For CA, the attacker must attack from the \arcrange{90}{-} horizontal arc.
