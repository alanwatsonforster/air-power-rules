...
\paragraph{\protect\CX{Set Up Information}{Set-Up}.} \CX{
...
\relevantadvancedrules{\ref{rule:half-fps}.}
}
...
\paragraph{Requirements Extending Beyond a Game Turn.} Some game activities, like ground-attack aiming, are measured in fractions of an aircraft's speed. If such a period extends beyond a game turn, the higher speed in either game turn is used to compute the requirement.
}
...
At the start of an aircraft's move, it must select one of the following three types of flight: level, climbing, or diving. These represent the aircraft committing its nose to remain level, to move up, or to move down for the game turn. Write the appropriate code in the flight type line of the aircraft log. Once selected, aircraft may not change its flight type until the next game turn. Aircraft may perform only one type of flight each game turn.
}
...
In level flight, all FPs must be expended as HFPs. The code for level flight is “LVL.” Level flight is assumed if allowed and not otherwise specified by a player when they begin to move an aircraft. Although all FPs are HFPs, aircraft can lose one altitude level by taking a free descent (see rule~\ref{rule:free-descent}).
}
...
In diving flight, the aircraft selects a specific type, either an \changedin{2A}{2A-unloaded-dives}{unloaded, steep, or vertical Dive}{steep or vertical dive}, and determines the altitude loss in levels allowed for each VFP expended. A mixture of HFPs and VFPs can then be expended subject to the limits outlined in rule~\ref{rule:diving-flight}.
}
...
\item If an aircraft uses a half-roll and dive (see rule~\ref{rule:half-rolls-and-dives}) to enter a vertical dive from a zoom or sustained climb or a VIFF vertical pitch (see rule~\ref{rule:viff-vertical-pitch}) to enter a vertical dive from a vertical climb, it must expend all of its HFPs (1/3 of its FPs, according to Table~\ref{table:fractions}) before using any VFPs.
\end{itemize}
}
