...
\midrule
&&\plus{1} to all attacks, \plus{1} to weapons launches.\\
1&Airframe&No supersonic speeds allowed.\asteriskmark\\
...
1,2&Airframe&No supersonic flight.\asteriskmark~No BT or ET turns. Low roll rate. No high pitch rate. Add two to prep-move requirements for all maneuvers.\\
...
\midrule
&&\plus{1} to all attacks and weapons launches.
\\
\cmidrule{3-3}
1&Airframe&
...
It becomes a smoker.
The aircraft must check each game turn after flight for a fuel fire. On a die roll of \minusafter{1}, a fire starts.
\\
6 or 7&Avionics&
...
It must check for a flame-out each time it changes its power setting. On a \minusafter{1}, one good engine flames out.
...
&&\plus{3} to all attacks and weapon launches.\\
\cmidrule{3-3}
1&Cockpit&
Roll the die again: 1 or 2 = pilot killed (equivalent to K). 3 or 4 = crewmember killed (lose multi-crew functions). \plusafter{5} = crew unhurt. 
...
It becomes a smoker.
It immediately checks twice per engine for a flame-out on a \minusafter{2}. On subsequent turns, if it changes its power setting, each engine flames out on a \minusafter{1}.
\\
\bottomrule
\end{tabularx}
