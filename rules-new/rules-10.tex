...
\CX{
The optional Damage Tables may be substituted for the generic damage level restrictions. These tables determine specific effects of L, H, and C level hits. For each hit consult \changedin{1C}{1C-tables}{the appropriate Damage Table}{Table~\ref{table:optional-damage}} and roll the die with no modifiers. \deletedin{1C}{1C-tables}{(Optional damage tables are in play aids sheets.)} \addedin{2B}{2B-optional-damage-tables}{Use the section of the table for the actual damage inflicted, not the cumulative damage. For example, if an aircraft already has H+2L damage and receives a further L damage, roll on the L table even though the cumulative damage will be C.}
