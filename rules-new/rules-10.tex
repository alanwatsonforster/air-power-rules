...
\begin{itemize}
\item If the target already has L or worse damage, apply a \plus{1} modifier.
\item If the hit was from a gun snap shot, apply a \minus{1} modifier.
\end{itemize}
...
\begin{itemize}
    ...
    \item For direct missile hits, apply a modifier of \minus{2}. Proximity hits do not receive this modifier.    
    
    \item For rocket hits, apply a modifier of \minus{2} just as for direct missile hits.
...
\begin{itemize}
    ...
    \item\itemparagraph{L or 2L.} Light damage. The aircraft suffers minor performance losses.

    \item\itemparagraph{H.} Heavy damage. The aircraft suffers major performance losses and possibly a system loss.

    \item\itemparagraph{C.} Crippled. The aircraft suffers severe performance losses, possibly a system loss, and is likely ineffective for further combat.
    
    \item\itemparagraph{K.} Killed. The aircraft is destroyed and removed from play.
...
\CX{
The optional Damage Tables may be substituted for the generic damage level restrictions. These tables determine specific effects of L, H, and C level hits. For each hit consult \changedin{1C}{1C-tables}{the appropriate Damage Table}{Table~\ref{table:optional-damage}} and roll the die with no modifiers. \deletedin{1C}{1C-tables}{(Optional damage tables are in play aids sheets.)} \addedin{2B}{2B-optional-damage-tables}{Use the section of the table for the actual damage inflicted, not the cumulative damage. For example, if an aircraft already has \tightbinaryplus{H}{2L} damage and receives a further L damage, roll on the L table even though the cumulative damage will be C.}
...
When an aircraft suffers damage, roll the die with no modifiers and consult Table~\ref{table:optional-damage}. Use the section appropriate for damage for the actual damage inflicted, not the cumulative damage. For example, if an aircraft already has \tightbinaryplus{H}{2L} damage and receives a further L damage, roll on the L table even though the cumulative damage will be C.
...
An aircraft suffering a fuel fire must check for the consequences at the end of the flight phase. Roll one die. On a \minusafter{2}, the aircraft explodes, killing the crew. On an \plusafter{8}, the fire goes out permanently, but the aircraft continues to leak fuel.

A fuel fire may be extinguished by shutting down all engines. Declare this when doing damage control, and treat the aircraft as if all engines were flamed out. Check for success at the end of the flight phase. Roll one die. On \minusafter{4}, the fire resumes permanently. Otherwise, it is extinguished. If the engines are subsequently restarted, the afterburner is disabled.
