...
\paragraph{Acceleration Limits.} If an aircraft is in level or climbing flight, Accel points that would push it beyond its maximum speed on the MMVC (for the altitude band in which it ends its flight) are unusable and ignored. If an aircraft in level or climbing flight is at its maximum speed, up to 1.5 Accel points \addedin{2B}{2B-raa-fp-carry}{(1.0 for RA aircraft) }may be carried forward; excess Accel are lost (because they would accelerate the aircraft beyond its maximum speed).
...
\begin{itemize}
   ...
   \item It requires 3 APs per 0.5 of speed gain, unless it is has rapid acceleration, in which case it requires 2 APs per 0.5 of speed gain. For example, a supersonic aircraft without rapid acceleration has accumulated 6.0 APs and 2.0 DPs, and so has a net of 4.0 APs. Its speed is increased by 0.5, and 1.0 AP is carried forward to the next game turn. If it had rapid acceleration, its speed would increase by 1.0, and 0.0 AP would be carried forward to the next game turn.
    ...
    \item If it uses turning flight (even at the EZ rate), it receives \CY[3A-turn-drag]{1.0 additional DP for the game turn}{0.5 additional DPs for each turn declared}.
...
\begin{itemize}
    ...
    It it uses turning flight (even at the EZ rate), it receives \CY[3A-turn-drag]{2.0 additional DPs for the game turn instead of 1.0}{1.0 DPs for each turn declared instead of 0.5}.
    }
...
\begin{itemize}
    ...
    If is uses turning flight, it receives \CY[3A-turn-drag]{no additional DPs for the game turn instead of 1.0}{no additional DPs for each turn declared instead of 0.5}.
    }
...
The first attempt succeeds on a die roll of \minusafter{2}, and the second and third attempts succeed on a die roll of \minusafter{4}. 
