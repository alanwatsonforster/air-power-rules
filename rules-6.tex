...
\begin{itemize}
    \item\itemparagraph{Idle.} This is the minimum setting that will keep a jet engine functioning; it provides no accel points, but on the turn in which Idle power is selected, the aircraft start speed is immediately reduced by the FP amount shown on the power chart. (This is the only game action that will change an aircraft's speed within the same game turn before it moves.)
...
Reminder:  Do not confuse Accel and Decel points with \changedin{1B}{JDW in the APJ 23 errata}{speed}{flight} points. They are separate things.
...
If the aircraft recovers, it may resume normal flight. Start speed is automatically minimum speed (from the MMVC) or the speed at which it departed (whichever is greater). On the turn of recovery, an aircraft may only choose diving flight (vertical dives are allowed). Exception: a High Pitch Rate capable aircraft may choose level flight. \addedin{1B}{JDW in APJ 35 QA}{The aircraft recovers to a wings level attitude.} \addedin{1B}{JDW in APJ 34 QA}{Any carried half FP is lost.}
...
\addedin{1B}{FH in APJ 36 errata}{The decel for transonic or M1 speed is assessed at the start of the turn, based on the aircrafts's starting speed and altitude.}
...
\begin{itemize}
    ...
    \itemaddedin{1B}{JDW in the APJ 23 errata and APJ 38 QA}{Aircraft at supersonic speeds require 3 accel per 0.5 speed gain unless they are Rapid Accel, in which case they now require the normal two accel per 0.5 speed.}
    
    \item \changedin{1B}{FH in APJ 36 errata}{An aircraft selecting Idle power loses 0.5 FPs of speed over that listed on the Power Setting Chart.  In addition, \changedin{1B}{JDW in the APJ 23 errata}{the aircraft is subject to the normal 1.0 decel point for being over cruise speed while at idle power}{it suffers decel penalties as if it were in Normal power.}.}{Aircraft at supersonic speeds may not select idle power.}

    \item An aircraft in Normal power receives 2.0 Decel per 0.5 FP of speed over High Transonic. In addition, the aircraft gets 1.0 Decel for flying at greater than cruise speed.

    \item An aircraft in Military power receives \changedin{1B}{JDW in the APJ 23 errata}{1.0}{1.5} decel per 0.5 FP of speed over High Transonic.

    \item An aircraft in Afterburner power is not penalized as for other power settings.

    \item \changedin{2A}{2A:spbr and AWF on the basis of that $1 \mathrm{FP} = 2\math{DP}$ used elsewhere, although MP in v2.4 has 4 DPs here.}{An aircraft which uses Speedbrakes may lose up to 0.5 FPs of speed over the amount listed in the power chart.}{An aircraft that uses speedbrakes may incur 1 DP more than the maximum listed in the ADC.}
...
\addedin{1B}{FH in APJ 36 errata}{The supersonic maneuver costs and penalties depend on the aircraft's supersonic status at the start of a maneuver.}
