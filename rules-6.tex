...
\paragraph{Insufficient Power Aboce Cruise Speed.} If an aircraft uses idle or normal power, and its speed exceeds its listed cruise speed, it receives one DP in addition to any others received that turn.
...
Aircraft flying at speeds approaching or exceeding the speed of sound are affected by the buildup of sonic shock waves and may receive DPs and restrictions as described below.
}
...
\begin{itemize}
   ...
   \item It requires 3 APs per 0.5 of speed gain unless it is an RA aircraft, which requires 2 APs per 0.5 of speed.
...
\addedin{2A}{2A-cruise}{\section{Realistic Cruise Speed} 
\label{rule:cruise-speed}

The cruise speed of an aircraft depends on its configuration:
\begin{itemize}
    \item If the configuration is CL, use the cruise speed given on the ADC.
    \item If the configuration is $1/2$, use the cruise speed on the ADC reduced by 0.5. 
    \item If the configuration is DT, use the cruise speed on the ADC reduced by 1.0.
\end{itemize} 

\addedin{2B}{AWF}{If idle or normal power is selected, the determination of whether the aircraft receives DPs for being above the cruise speed uses the configuration at the start of a game turn.}
}
